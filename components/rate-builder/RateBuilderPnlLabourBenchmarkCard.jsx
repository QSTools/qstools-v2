...
export default function RateBuilderPnlLabourBenchmarkCard({
  ...
  const pnl_recovery_gap_to_labour_cost =
    pnl_implied_recovered_rate - source_cost_rate;

  const pnl_recovery_is_above_labour_cost =
    pnl_benchmark_ready && pnl_recovery_gap_to_labour_cost >= 0;
        ...
        <div className="rate-builder-labour-breakdown">
          ...
          {has_labour_recovery_leak ? (
            <p className="ui-help">
              ...
              labour cost. The remaining{" "}
              {(unrecovered_labour_percent * 100).toFixed(1)}% is not being
              recovered by the current P&amp;L structure.
            </p>
          ) : null}
          ...
          <div className="rate-builder-labour-breakdown__row">
            <span>P&amp;L recovery vs labour cost</span>
            <strong>
              {pnl_benchmark_ready
                ? pnl_recovery_is_above_labour_cost
                  ? `Surplus: ${format_currency(
                      pnl_recovery_gap_to_labour_cost
                    )} / ${labour_unit_label}`
                  : `Leak: ${format_currency(
                      Math.abs(pnl_recovery_gap_to_labour_cost)
                    )} / ${labour_unit_label}`
                : "Not ready"}
            </strong>
          </div>
          ...
          <p className="ui-help">
            ...
            P&amp;L leaves {format_currency(labour_recovery_pool)} available to
            recover labour after COG, net profit, and non-labour overheads are
            removed.
          </p>
        </div>
      ) : null}
