...
function AssetPickerSection({
  ...
  return (
    <div className="ui-stack-sm">
      <div>
        <p className="cost-allocation-assignment-title">{title}</p>
        <p className="cost-allocation-assignment-help">{help_text}</p>
      </div>
...
export default function CostAllocationGroupAssetBuilder({
  ...
  const productive_rows = getAssetRows(asset_assignment);

  const support_rows = getSupportAssetRows(asset_assignment);

  const all_group_assignments = getAssetAssignments(asset_assignment, group_id);

  const { productive: productive_assignments, support: support_assignments } =
    splitAssetAssignmentsByType(all_group_assignments, asset_assignment);

  return (
    <div className="cost-allocation-assignment-block">
      <AssetPickerSection
        title="Productive assets"
        help_text="Add productive asset allocation into this group. Each asset can only be allocated up to 100% across all operating groups."
        rows={productive_rows}
        asset_assignment={asset_assignment}
        group_id={group_id}
        add_asset_assignment={add_asset_assignment}
        remove_asset_assignment={remove_asset_assignment}
        assignments={productive_assignments}
        empty_message="No assets assigned to this group yet."
      />

      {support_rows.length > 0 || support_assignments.length > 0 ? (
        <div className="cost-allocation-assignment-block-divider">
          <AssetPickerSection
            title="Non-productive assets"
            help_text="Cost only — these do not contribute recovery hours or a recovery rate. Use this for support assets like a ute or manager's car that are part of this group's real cost but are never charged out."
            rows={support_rows}
            asset_assignment={asset_assignment}
            group_id={group_id}
            add_asset_assignment={add_asset_assignment}
            remove_asset_assignment={remove_asset_assignment}
            assignments={support_assignments}
            empty_message="No non-productive assets assigned to this group yet."
          />
        </div>
      ) : null}
    </div>
