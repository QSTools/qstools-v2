...
export default function FixedAssetRegisterImportPanel({ onImportFile, is_importing, import_error, has_data, source_filename, imported_at }) {
  ...
  return (
    <div className="ui-panel ui-stack-sm">
      <div className="ui-kicker">Fixed Asset Register</div>
      <div className="ui-card-title-sm">Import from Xero</div>
      <p className="ui-help">
        In Xero: Accounting &rarr; Fixed Assets &rarr; Export. Upload that file here. Disposed assets are
      ...
      <input ref={file_input_ref} type="file" accept=".xlsx" onChange={handle_file_change} />
      ...
      >
        {is_importing ? "Importing..." : "Import Fixed Asset Register"}
      </button>

      {import_error && (
        <p className="ui-help" style={{ color: "var(--danger)" }}>{import_error}</p>
      )}

      {last_import_message && (
        <p className="ui-help" style={{ color: "var(--success)" }}>{last_import_message}</p>
      )}

      {last_import_warnings.length > 0 && (
        <div className="ui-help" style={{ color: "var(--warning)" }}>
          <strong>{last_import_warnings.length} warning{last_import_warnings.length === 1 ? "" : "s"}:</strong>
          <ul>
