...
REPLACEMENTS: Dict[str, str] = {
    ...
    "focus:border-neutral-500": "focus:border-[var(--accent)]",
    "hover:bg-neutral-800": "hover:bg-[var(--bg-card-muted)]",
}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replace Labour hardcoded theme classes with CSS var tokens."
    )
    ...
    for file_path in iter_files(root):
        ...
        for old, count in found.most_common():
            print(f"  {old:<30} -> {REPLACEMENTS[old]}   ({count})")
