...
// Legacy baseline/upside/downside scenario system, fed from
// useBusinessSummary(). Extracted out of useBusinessModelling.js
// (2026-09-16 refactor - pure extraction, no behaviour change) because
// that file had grown to cover three separate jobs in one place. This
// hook receives business_summary as a PARAMETER rather than calling
// useBusinessSummary() itself, so the orchestrator hook only fetches it
// once and passes it down - do not call useBusinessSummary() again in
// here, that would create a second, redundant subscription.
//
// Rendered on the page collapsed as "Previous scenario tools" - left in
// place as a safety net alongside the newer live-lever system and the
// Baseline/Upside/Downside scenario redesign (useBusinessModellingScenario.js),
// per the 2026-09-12 decision lock. Not yet retired - that is a separate,
...
export default function useBusinessModellingLegacyScenario(business_summary) {
  // Preserved from the original file - result unused directly, but the
  // hook call itself is kept as-is so this extraction changes nothing
  // about what runs. Do not remove without separately confirming it is
  // genuinely safe to.
