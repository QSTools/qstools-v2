...
function normaliseRegisterAsset(asset = {}) {
  return {
    ...
    // Whether this register asset should appear as a linkable
    // suggestion on the Assets module's Asset Name field. Default
    // false ("default unticked" per user decision 2026-09-12) - a
    // real, PERSISTED flag, not local component state, so AssetForm
    // (a different page) can actually read it. Was matched_asset_id
    // in an earlier, wrong-direction design - corrected same session.
    is_included: asset.is_included === true,
  };
}
