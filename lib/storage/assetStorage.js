...
const ASSET_UTILISATION_WEEKS_PER_YEAR = 48;
...
export function normalizeAssetType(value) {
  ...
  if (
    ...
    normalized_value === "non_productive" ||
    normalized_value === "nonproductive"
  ) {
    return "support";
  }

  if (normalized_value === "productive") {
    return "productive";
  }
...
export function createEmptyAssetState() {
  ...
  return {
    ...
    scheduled_hours_per_week: null,
    downtime_hours_per_week: 0,
    utilisation_hours_per_week: 40,
    utilisation_hours_annual: 40 * ASSET_UTILISATION_WEEKS_PER_YEAR,

    purchase_price: 0,
    // Added 2026-09-12 for Fixed Asset Register linking - see
    // FIXED_ASSET_REGISTER_IMPORT_SCOPING_BRIEF_2026-09-12.txt. When
    // linked_register_asset_number is set, purchase_price was sourced
    // from a real imported Xero register entry (not typed in) and the
    // form should render it read-only - user decision: "can't invent a
    // number" once it's genuinely sourced. Both default to
    // unset/false, matching every existing asset today exactly -
    ...
    // scoping brief) - both default to unset, so nothing changes for
    // any existing asset. estimated_annual_depreciation is stored at
    // link time (not recomputed live) using the same validated formula
    // as the /imports preview table (fixedAssetRegisterCalculations.js).
...
export function sanitizeAssetState(input = {}) {
  ...
  const asset_type = normalizeAssetType(input.asset_type);
  const default_utilisation_hours_per_week =
    asset_type === "productive" ? 40 : 0;

  const has_scheduled_hours =
    input.scheduled_hours_per_week !== undefined &&
    input.scheduled_hours_per_week !== null &&
    input.scheduled_hours_per_week !== "";

  const scheduled_hours_per_week_raw = has_scheduled_hours
    ? Number(input.scheduled_hours_per_week)
    : null;

  const safe_scheduled_hours_per_week =
    scheduled_hours_per_week_raw !== null &&
    Number.isFinite(scheduled_hours_per_week_raw)
      ? Math.max(scheduled_hours_per_week_raw, 0)
      : null;

  const downtime_hours_per_week_raw = Number(
    input.downtime_hours_per_week || 0
  );
  const safe_downtime_hours_per_week = Number.isFinite(
    downtime_hours_per_week_raw
  )
    ? Math.max(downtime_hours_per_week_raw, 0)
    : 0;

  const utilisation_hours_per_week =
    asset_type === "productive"
      ? safe_scheduled_hours_per_week !== null
        ? Math.max(
            safe_scheduled_hours_per_week - safe_downtime_hours_per_week,
            0
          )
        : Number(
            input.utilisation_hours_per_week ??
              input.hours_used_per_week ??
              default_utilisation_hours_per_week
          )
      : 0;

  const safe_utilisation_hours_per_week = Number.isFinite(
    utilisation_hours_per_week
  )
    ? Math.max(utilisation_hours_per_week, 0)
    : default_utilisation_hours_per_week;
  ...
  return {
    ...
    scheduled_hours_per_week:
      asset_type === "productive" ? safe_scheduled_hours_per_week : null,
    downtime_hours_per_week:
      asset_type === "productive" ? safe_downtime_hours_per_week : 0,
    utilisation_hours_per_week: safe_utilisation_hours_per_week,
    utilisation_hours_annual:
      asset_type === "productive"
        ? safe_utilisation_hours_per_week * ASSET_UTILISATION_WEEKS_PER_YEAR
        : 0,

    purchase_price: Number(input.purchase_price || 0),
    linked_register_asset_number:
      typeof input.linked_register_asset_number === "string"
        ? input.linked_register_asset_number
        : "",
    purchase_price_locked: Boolean(input.purchase_price_locked),
    estimated_annual_depreciation:
      input.estimated_annual_depreciation === null || input.estimated_annual_depreciation === undefined
        ? null
        : Number(input.estimated_annual_depreciation),
    include_depreciation_in_cost: Boolean(input.include_depreciation_in_cost),
...
function sanitizeSavedAssetRecord(input = {}) {
  ...
  return {
    ...
    scheduled_hours_per_week: Number.isFinite(
      Number(sanitized.scheduled_hours_per_week)
    )
      ? sanitized.scheduled_hours_per_week
      : null,
    downtime_hours_per_week: Number(sanitized.downtime_hours_per_week ?? 0),
    utilisation_hours_per_week: Number(
      recalculated.utilisation_hours_per_week ??
        sanitized.utilisation_hours_per_week ??
        0
    ),
    utilisation_hours_annual: Number(
      recalculated.utilisation_hours_annual ??
        sanitized.utilisation_hours_annual ??
        0
    ),
    required_asset_recovery_rate: Number(
      recalculated.required_asset_recovery_rate ??
        input.required_asset_recovery_rate ??
        0
    ),
    productive_asset_hours: Number(
      recalculated.productive_asset_hours ??
        input.productive_asset_hours ??
        0
    ),
    true_asset_cost_per_hour: Number(
      recalculated.true_asset_cost_per_hour ??
        input.true_asset_cost_per_hour ??
        0
    ),
  };
}
