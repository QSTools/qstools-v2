...
function build_warnings(output_contract) {
  ...
  if (output_contract.excluded_asset_finance_interest_total > 0) {
    warnings.push(
      "Asset finance interest has been excluded from QS benchmark costs to prevent double counting.",
    );
  }
...
function build_unassigned_details(state) {
  const unassigned_lines = state.pnl_lines.filter(line => line.category === "unassigned");
  ...
  unassigned_lines.forEach(line => {
    ...
    if (line.line_name.toLowerCase().includes('wip') || line.line_name.toLowerCase().includes('adjustment')) {
