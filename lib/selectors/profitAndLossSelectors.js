...
function build_category_options() {
  return [
    ...
    { value: "cogs", label: "COGS / Direct Cost" },
...
function build_unassigned_details(state) {
  ...
  unassigned_lines.forEach(line => {
    const formatted_line = {
      ...
      current_category:
        normalise_category(line.category) === "review_required"
          ? "Review Required"
          : normalise_category(line.category) === "cogs"
            ? "COGS / Direct Cost Review"
            : "Unassigned",
      suggested_action: "",
    };
  ...
  return {
    ...
    summary: {
      ...
      direct_cost_category_totals:
        output_contract.direct_cost_category_totals?.map((category) => ({
          ...category,
          formatted_total: format_number(category.total),
        })) ?? [],
      total_direct_costs: format_number(output_contract.total_direct_costs),
