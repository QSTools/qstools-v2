function format_number(value, decimals = 1) {
  ...
  return Number.isFinite(parsed) ? parsed.toFixed(decimals) : "0.0";
}

function format_hours(value) {
  return `${format_number(value, 1)} hrs`;
}
...
export function build_opening_hours_status(opening_hours) {
  return {
    ...
    standard_weekly_open_hours: opening_hours.standard_weekly_open_hours,
...
export function build_opening_hours_card(opening_hours) {
  return {
    ...
    summary_rows: [
      {
        ...
        value: format_number(opening_hours.open_day_count, 1),
      },
      {
      ...
      },
      {
        ...
        value: format_number(opening_hours.annual_open_weeks, 1),
      },
      {
    ...
    downstream_rows: [
      ...
      {
        label: "Rate Builder",
        value: "Future use for capacity pressure and working-time context.",
      },
      {
        label: "Quote Checker",
        value:
