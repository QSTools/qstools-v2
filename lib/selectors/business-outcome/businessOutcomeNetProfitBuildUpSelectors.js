...
export function buildNetProfitBuildUpRows(selected_output) {
  ...
  // BO-1 (v6.0 8b): the non-productive share and the injected true cost live ONLY in
  // group_real_capacity (file map gotcha #4) - read them from there, never from rows.
  const real_capacity_by_group_id = new Map(
    safe_array(selected_output.real_capacity?.group_real_capacity).map((g) => [g.group_id, g])
  );
  ...
  const rows = safe_array(all_sources).map((source) => {
    if (source.type === "materials") {
      return {
        ...
        material_true_cost: to_number(selected_output.materials?.true_cost),
        // positive = materials shortfall absorbed by the groups (View A cascade)
        shortfall_carried_by_others: Math.round((to_number(source.net_profit) - (to_number(source.modelled_revenue) - to_number(selected_output.materials?.true_cost))) * 100) / 100,
        revenue_members: [],
      };
    }
    ...
    // BO-1: reconcile the shown parts (direct + overhead + non-productive) against the
    // injected true cost, and expose the cascade transfer so revenue less the lines = contribution.
    const rc = real_capacity_by_group_id.get(group_id) || null;
    const non_productive_cost = rc ? to_number(rc.non_productive_cost) : 0;
    const injected_true_cost = rc && rc.true_cost !== undefined ? to_number(rc.true_cost) : null;
    const injected_variance =
      injected_true_cost === null ? 0 : Math.round((regrouped_true_cost + non_productive_cost - injected_true_cost) * 100) / 100;
    const shortfall_absorbed =
      rc && rc.naive_net_profit !== null && rc.naive_net_profit !== undefined
        ? Math.round((to_number(rc.naive_net_profit) - to_number(source.net_profit)) * 100) / 100
        : 0;
    ...
    return {
      ...
      non_productive_cost,
      shortfall_absorbed,
      true_cost_reconciles: injected_true_cost === null ? true_cost_reconciles : Math.abs(injected_variance) < 1,
      true_cost_variance: injected_true_cost === null ? true_cost_variance : injected_variance,
      net_profit: to_number(source.net_profit),
