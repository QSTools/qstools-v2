...
function group_sources_by_group(sources, member_key) {
  ...
  sources.forEach((row) => {
    ...
    group.group_overhead_share += row.overhead_share ?? 0;
    const row_current_rate = row.blended_rate ?? row.charge_out_rate ?? null;
    if (group.current_rate === null && row_current_rate !== null && row_current_rate !== undefined) group.current_rate = row_current_rate;
    if (group.minimum_recoverable_rate === null && row.minimum_recoverable_rate_per_hour !== null && row.minimum_recoverable_rate_per_hour !== undefined) group.minimum_recoverable_rate = row.minimum_recoverable_rate_per_hour;
...
export function merge_groups_by_id(labour_groups, asset_groups, materials, use_implied) {
  ...
  labour_groups.forEach((g) => {
    ...
    entry.total_cost += g.group_true_cost ?? 0;
    entry.overhead_share += g.group_overhead_share ?? 0;
    if (g.current_rate !== null && g.current_rate !== undefined) {
      entry.current_rate_sum += g.current_rate;
      entry.current_rate_count += 1;
    }
    if (entry.minimum_recoverable_rate === null && g.minimum_recoverable_rate !== null && g.minimum_recoverable_rate !== undefined) entry.minimum_recoverable_rate = g.minimum_recoverable_rate;
  ...
  const merged_entries = Array.from(by_group_id.values()).map((entry) => {
    const current_rate = entry.current_rate_count > 0 ? entry.current_rate_sum / entry.current_rate_count : null;
...
export function merge_groups_by_id_real_capacity(labour_groups, asset_groups, materials, group_real_capacity = null) {
  const by_group_id = new Map();
  ...
  labour_groups.forEach((g) => {
    ...
    g.staff.forEach((s) => {
      ...
      const child_true_cost = (s.true_cost ?? 0) + (s.non_productive_share ?? 0);
      const child_net_profit = s.real_capacity_net_profit ?? 0;
      const child_achieved_revenue = child_true_cost + child_net_profit;
      const child_hours = s.hours ?? 0;
      const child_current_rate = s.charge_out_rate ?? s.blended_rate ?? null;
      entry.children.push({
        key: s.staff_type_id,
        label: s.staff_type_name,
        ...
        verdict,
        verdict_label: verdict_label(verdict),
        total_cost: child_true_cost,
        overhead_share: s.overhead_share ?? 0,
        current_rate: child_current_rate,
        minimum_recoverable_rate: s.minimum_recoverable_rate_per_hour ?? null,
  ...
  asset_groups.forEach((g) => {
    ...
    g.assets.forEach((a) => {
      ...
      const child_true_cost = (a.true_cost ?? 0) + (a.non_productive_share ?? 0);
      const child_net_profit = a.real_capacity_net_profit ?? 0;
      const child_achieved_revenue = child_true_cost + child_net_profit;
      const child_hours = a.hours ?? 0;
      const child_current_rate = a.blended_rate ?? a.charge_out_rate ?? null;
      entry.children.push({
        key: a.asset_id,
        label: a.asset_name,
        net_profit: child_net_profit,
        ...
        verdict_label: verdict_label(verdict),
        total_cost: child_true_cost,
        overhead_share: a.overhead_share ?? 0,
        current_rate: child_current_rate,
        minimum_recoverable_rate: a.minimum_recoverable_rate_per_hour ?? null,
        achieved_rate: child_hours > 0 ? child_achieved_revenue / child_hours : null,
        achieved_hours: child_current_rate > 0 ? child_achieved_revenue / child_current_rate : null,
      });
    });
  ...
  // BO-1 (v6.0 8b): entry.total_cost above sums raw per-row true_cost, which never
  // carries the non-productive share. When the cascade's group figures are passed,
  // use their injected true_cost instead (file map gotcha #4).
  if (Array.isArray(group_real_capacity)) {
    const injected_by_id = new Map(group_real_capacity.map((g) => [g.group_id, g]));
    by_group_id.forEach((entry) => {
      const g = injected_by_id.get(entry.key);
      if (!g || g.true_cost === undefined || g.true_cost === null) return;
      entry.non_productive_cost = Number(g.non_productive_cost) || 0;
      entry.total_cost = Number(g.true_cost) || 0;
    });
  }
  const merged_entries = Array.from(by_group_id.values()).map((entry) => {
    const current_rate = entry.current_rate_count > 0 ? entry.current_rate_sum / entry.current_rate_count : null;
...
export function selectBusinessOutcomePerSourceRevenue(per_source_result) {
  ...
  const {
    ...
    // See the matching fix in useBusinessOutcomePerSourceRevenue.js's
    // own total_true_cost - this is the same fix applied at the
    // headline net profit layer.
  ...
  // Full list, everyone - sorted worst to best, same convention as
  // being_carried above - used by Card 1's "click a source to see its
  // own breakdown" list (this session), so problems surface first but
  // every source is visible, not just the failing ones.
  const all_sources = [...all_group_level_entries].sort((a, b) => a.net_profit - b.net_profit);
  ...
  const all_group_level_entries_real_capacity = merge_groups_by_id_real_capacity(
    labour_groups,
    asset_groups,
    materials, per_source_result.real_capacity?.group_real_capacity).map((entry) => ({
    key: entry.key,
    name: entry.label,
    net_profit: entry.net_profit,
    modelled_revenue: entry.modelled_revenue,
  ...
  // REVERTED (2026-09-17): assigned_non_productive_*_cost is no longer
  // subtracted here - total_source_net_profit_real_capacity already
  // reflects it, since each working unit's own real_capacity_net_profit
  // now includes its non-productive cost via the cascade fix in
  // useBusinessOutcomePerSourceRevenue.js. Subtracting it again here
  // would double-count it. The plain (naive) total_net_profit below
  // still needs its own subtraction - that path is unaffected by the
  // cascade, nothing else accounts for it there.
  ...
  const all_sources_real_capacity = [...all_group_level_entries_real_capacity].sort(
  ...
  const headline_real_capacity = {
    ...
    all_sources: all_sources_real_capacity,
  ...
  // VIEW B HEADLINE (2026-09-18) - built for Business Modelling's real-
  // engine rerun, NOT the display layer (View B's own display already
  // has merge_view_b_groups in BusinessOutcomePerSourceRevenueCard.jsx -
  // deliberately not reused here, since a selector importing from a UI
  // component would be backwards, and Business Modelling only needs
  // this simpler shape, not the full display shape with children).
  // View B is the only cascade where materials has genuinely
  // independent, lever-movable economics - View A's materials is a
  // residual with nothing for a markup override to affect. Sourced
  // entirely from view_b.real_capacity.group_real_capacity, already
  // correct and already tested (2026-09-18 View B parity fix) - this
  // is purely additive reshaping of already-verified data, no new
  // calculation. Materials is included as just another entry here
  // (its group_id is "materials"), matching View B's own "genuine
  // sixth peer" philosophy - no special-casing needed.
  const view_b = per_source_result.view_b;
  let view_b_headline_real_capacity = null;
  if (view_b && view_b.real_capacity) {
    const view_b_group_entries = (view_b.real_capacity.group_real_capacity || []).map((g) => {
      const net_profit = g.final_net_profit;
      return {
        key: g.group_id,
        name: g.group_name,
        net_profit,
        modelled_revenue: g.modelled_revenue ?? 0,
        verdict: net_profit >= 0 ? "paying_its_way" : "being_carried",
        type: g.group_id === "materials" ? "materials" : "group",
      };
    });
    const view_b_being_carried = view_b_group_entries.filter((e) => e.verdict === "being_carried");
    const view_b_total_net_profit = view_b_group_entries.reduce((sum, e) => sum + (e.net_profit ?? 0), 0);
    // BO-11 / F8 (a), v6.0 8b: real revenue ABOVE what rates + markup model is real money.
    // The View B cascade already returns it as surplus; count it in the headline and
    // publish it separately ("revenue your rates don't explain"). Zero whenever the
    // ceiling binds (real revenue below modelled), so nothing changes in that case.
    const view_b_unexplained_revenue = Math.max(0, Number(view_b.real_capacity?.surplus) || 0);

    view_b_headline_real_capacity = {
      total_net_profit: view_b_total_net_profit + view_b_unexplained_revenue,
      unexplained_revenue: view_b_unexplained_revenue,
      total_modelled_revenue: total_revenue_reference,
      independent_modelled_revenue: view_b.revenue_ceiling?.all_peers_modelled_total ?? 0,
      total_group_count: view_b_group_entries.length,
      being_carried_count: view_b_being_carried.length,
      being_carried: view_b_being_carried,
      all_sources: view_b_group_entries,
      all_good: view_b_being_carried.length === 0,
      labour_capacity_warning: per_source_result.labour_pool_over_allocated === true,
      asset_capacity_warning: per_source_result.asset_pool_over_allocated === true,
      labour_coverage_gaps: per_source_result.labour_coverage_gaps || [],
    };
  ...
  // ASSUMED CAPACITY ledger groups (this session) - reuses
  // merge_groups_by_id (already the single source of truth for both
  // headline and drill) called twice, once naive and once implied, then
  // zipped together by key. No new aggregation logic - the same,
  // already-verified merge just run both ways and compared.
  const assumed_ledger_naive = merge_groups_by_id(labour_groups, asset_groups, materials, false);
  const assumed_ledger_implied = merge_groups_by_id(labour_groups, asset_groups, materials, true);
  const assumed_ledger_groups = assumed_ledger_naive
    .filter((e) => e.type === "group")
    .map((naive_entry) => {
      const implied_entry = assumed_ledger_implied.find((e) => e.key === naive_entry.key);
      return {
        group_id: naive_entry.key,
        group_name: naive_entry.label,
        naive_net_profit: naive_entry.net_profit,
        implied_net_profit: implied_entry ? implied_entry.net_profit : naive_entry.net_profit,
        verdict: implied_entry ? implied_entry.verdict : naive_entry.verdict,
      };
    });
  ...
  return {
    ...
    // tied to any revenue-bearing source (non-productive labour/
    // assets) - already subtracted in total_net_profit/
    // total_net_profit_real_capacity above; exposed here too so the
    // Card's breakdown list can show it as its own row and keep
    // summing to the real headline total. Deliberately separate from
    // "unassigned" above - this cost IS correctly assigned, it just
    // has no revenue-bearing source, so it must not imply there is
    // still something to go fix in Cost Allocation.
    non_revenue_bearing: {
    ...
    pure_support_groups: per_source_result.pure_support_groups ?? [],
    non_productive_cost_distributed_total: per_source_result.non_productive_cost_distributed_total ?? 0,

    residual_overhead,
    view_b: per_source_result.view_b ?? null,
    capacity_coverage_gap: per_source_result.capacity_coverage_gap ?? null,
    health_gauge: per_source_result.health_gauge ?? null,
