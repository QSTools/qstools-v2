...
export function selectBusinessOutcomeDisplay(calculation, trustStates) {
  ...
  const reconciliation = labourSplit?.reconciliation ?? null;
  const labourReconciliationRows = reconciliation
    ? [
        { id: 'labour_only', label: 'Labour-only groups (above)', value: reconciliation.labourOnlyGroupsCost },
        { id: 'labour_in_asset_groups', label: 'Labour inside Asset-backed groups', value: reconciliation.labourInAssetGroupsCost, note: 'see Assets' },
        { id: 'non_productive', label: 'Non-Productive Labour', value: reconciliation.nonProductiveLabourCost, note: 'never charged' },
        { id: 'unassigned', label: 'Unassigned Labour', value: reconciliation.unassignedLabourCost, note: reconciliation.unassignedLabourCost > 0 ? 'coverage gap' : 'fully covered' },
        { id: 'total', label: 'Total Labour', value: reconciliation.totalLabourAccounted, isTotal: true },
      ]
    : [];

  // Same blended rate applied uniformly to every labour-only group -
  // pulled from the first priced row, since it's identical across all.
  ...
  if (reconciliation && reconciliation.reconciles === false) {
    warnings.push({
      id: 'labour_reconciliation_failed',
      severity: 'warning',
      message: `Labour reconciliation does not match: accounted $${reconciliation.totalLabourAccounted.toFixed(0)} vs actual $${reconciliation.totalLabourCost.toFixed(0)} - variance $${reconciliation.variance.toFixed(0)}. Do not trust the Labour Split breakdown until this is resolved.`,
    });
  }
  ...
  return {
    ...
    labourReconciles: reconciliation?.reconciles ?? null,
