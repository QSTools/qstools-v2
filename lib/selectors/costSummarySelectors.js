...
export function selectCostSummaryLabour(labour) {
  ...
  const fallback_staff = profiles.map((profile) => {
    ...
    return {
      ...
      charge_out_rate: toNumber(data?.charge_out_rate ?? outputs?.charge_out_rate),
...
export function buildCostSummaryStatus({
  ...
  if (
    ...
    warnings.push("No employee overheads linked");
  }
  ...
  if (!has_asset_data) {
    warnings.push("No asset costs linked");
  }

  if (!has_general_overhead_data) {
    warnings.push("No general overheads linked");
  }
