...
export function selectCostSummaryLabour(labour) {
  ...
  const stored_staff = profiles.map((profile) => {
    ...
    const annual_gross_wages = toNumber(outputs?.annual_gross_wages);
...
export function buildCostSummaryCard({ labour_data, calculations }) {
  const people_rows = [...labour_data.active_staff]
    .map((staff) => ({
      ...
      gross_wages_total: staff.annual_gross_wages,
      entitlement_cost_total: staff.entitlement_cost_total,
      esct_total: staff.esct_total,
      employee_overheads_total: 0,
      total_people_cost:
        toNumber(staff.annual_gross_wages) +
        toNumber(staff.entitlement_cost_total) +
        toNumber(staff.esct_total),
      charge_out_rate: staff.charge_out_rate,
    }))
    .sort((a, b) => b.total_people_cost - a.total_people_cost);

  const gross_wages_total = people_rows.reduce(
    (sum, row) => sum + toNumber(row.gross_wages_total),
    0
  );
  ...
  const employee_overheads_total = 0;
  ...
  const labour_share =
    calculations.total_cost_burden > 0
      ? (calculations.total_labour_cost_annual / calculations.total_cost_burden) *
        100
      : 0;
