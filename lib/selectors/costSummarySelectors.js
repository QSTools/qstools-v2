...
export function buildCostSummaryStatus({
  ...
  if (!has_labour_data) {
    warnings.push("Labour output contract is not flowing into Cost Summary.");
    missing_modules.push("Labour");
  }

  if (!has_asset_data) {
    warnings.push("Assets output contract is not flowing into Cost Summary.");
    missing_modules.push("Assets");
  }

  if (!has_general_overhead_data) {
    warnings.push("General Overheads output contract is not flowing into Cost Summary.");
...
export function buildCostSummaryCard({
  ...
  let highlight_insight =
    "Cost Summary is consuming aligned upstream output contracts.";
