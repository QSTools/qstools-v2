...
function getBusinessRecoveryHours({
  total_productive_output = 0,
  productive_staff_count = 0,
  total_recovery_hours = 0,
}) {
  // F18: recovery hours are the whole productive team's hours (person-hours),
  // not one average person's. Pricing-side rates (Recovery Summary, and via
  // required_recovery_rate Business Modelling / Quote Engine) divide by these.
  // The macro per-open-hour view uses Opening Hours instead and is unaffected.
  // productive_staff_count is kept in the signature for callers but no longer
  // divides the total.
  if (total_productive_output > 0) {
    return total_productive_output;
  }

  return toNumber(total_recovery_hours);
}
...
export function selectCostSummaryLabour(labour) {
  ...
  const total_staff_recovery_hours = toNumber(
    output_contract.total_recovery_hours
  );
  const business_recovery_hours = getBusinessRecoveryHours({
    total_productive_output,
    productive_staff_count: labour_detail.productive.staff_count,
    total_recovery_hours: total_staff_recovery_hours,
  });
  ...
  return {
    ...
    operating_recovery_hours: business_recovery_hours,
    total_recovery_hours: business_recovery_hours,
...
export function buildCostSummaryStatus({
  ...
  // Propagate opening hours warnings from calculations, if present
  if (Array.isArray(calculations?.opening_hours_warnings)) {
    calculations.opening_hours_warnings.forEach((w) => warnings.push(w));
  }
...
export function buildCostSummaryCard({
  ...
  return {
    ...
    required_labour_burden_rate: calculations.required_labour_burden_rate,
    macro_required_operating_hour_rate:
      calculations.macro_required_operating_hour_rate,
    net_annual_business_open_hours: calculations.net_annual_business_open_hours,
    net_annual_business_open_days: calculations.net_annual_business_open_days,
    annual_open_weeks: calculations.annual_open_weeks,
    total_recovery_hours: calculations.total_recovery_hours,
    business_recovery_hours: calculations.business_recovery_hours,
    operating_recovery_hours: calculations.operating_recovery_hours,
    total_staff_recovery_hours: calculations.total_staff_recovery_hours,
