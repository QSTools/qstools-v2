...
export function buildBusinessModellingStatus({
  ...
  const readiness_blocked =
    model_readiness_status?.is_ready_for_modelling === false ||
    model_readiness_status?.overall_status === "blocked" ||
    model_readiness_status?.model_readiness_status === "blocked";

  const warning_count =
    modelling_warnings.length +
    (Array.isArray(model_readiness_status?.warning_items)
      ? model_readiness_status.warning_items.length
      : 0);

  const status_label =
    readiness_blocked || !modelling_ready
      ? "blocked"
      : warning_count > 0
        ? "ready_with_warnings"
        : "ready";
  ...
  return {
    ...
    warning_count,
    modelling_ready: modelling_ready && !readiness_blocked,
    modelling_warnings,
    ...
    model_readiness_gate_status:
      ...
      "blocked",
    model_readiness_ready_for_modelling:
      model_readiness_status?.is_ready_for_modelling === true,
    model_readiness_ready_for_ai_export:
      model_readiness_status?.is_ready_for_ai_export === true,
    model_readiness_ready_for_dashboard:
      model_readiness_status?.is_ready_for_dashboard === true,
    model_readiness_blockers: model_readiness_status?.blocking_items ?? [],
    model_readiness_warnings: model_readiness_status?.warning_items ?? [],
  };
}
...
export function buildBusinessModellingScenarioControls(scenario) {
  ...
  return {
    ...
    scenario_charge_out_rate: toNumber(source.scenario_charge_out_rate),
    scenario_productivity_percent: toNumber(
      source.scenario_productivity_percent
    ),
    baseline_productivity_percent: toNumber(
      source.baseline_productivity_percent
    ),
    scenario_margin_target_percent: toNumber(
      source.scenario_margin_target_percent
    ),
    scenario_billable_volume_change_percent: toNumber(
      source.scenario_billable_volume_change_percent
    ),
  };
}
...
export function buildBusinessModellingDeltaCard({
  ...
  return {
    ...
    net_position_change:
      toNumber(source.scenario_net_position) - toNumber(baseline.net_position),
    margin_pool_change:
      toNumber(source.scenario_margin_pool) - toNumber(baseline.margin_pool),
    recovery_gap_change:
...
export function buildBusinessModellingSelectedModelOutput({
  ...
  return {
    ...
    selected_model_name:
      selected?.scenario_name || selected?.baseline_name || "Baseline",
    selected_model_type: selected_type,
    selected_model_trust_state:
      selected?.scenario_trust_state || selected?.model_trust_state || "blocked",
    selected_model_total_revenue:
      toNumber(selected?.scenario_total_revenue) ||
      toNumber(selected?.total_revenue),
    selected_model_direct_costs:
      toNumber(selected?.scenario_total_direct_costs) ||
      toNumber(selected?.total_direct_costs),
    selected_model_margin_pool:
      toNumber(selected?.scenario_margin_pool) ||
      toNumber(selected?.margin_pool),
    selected_model_total_cost_burden:
      toNumber(selected?.scenario_total_cost_burden) ||
      toNumber(selected?.total_cost_burden),
    selected_model_total_productive_output:
      toNumber(selected?.scenario_total_productive_output) ||
      toNumber(selected?.total_productive_output),
    selected_model_required_recovery_rate:
      toNumber(selected?.scenario_required_recovery_rate) ||
      toNumber(selected?.required_recovery_rate),
    selected_model_margin_per_productive_hour:
      toNumber(selected?.scenario_margin_per_productive_hour) ||
      toNumber(selected?.current_margin_per_productive_hour),
    selected_model_recovery_gap_per_hour:
      toNumber(selected?.scenario_recovery_gap_per_hour) ||
      toNumber(selected?.recovery_gap_per_hour),
    selected_model_net_position:
      toNumber(selected?.scenario_net_position) ||
      toNumber(selected?.net_position),
  };
}
