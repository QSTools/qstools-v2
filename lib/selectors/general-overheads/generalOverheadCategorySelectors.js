...
// Reconciliation "show the maths" layer (Model Reconciliation page)
// needs both sides of each category: what the P&L says (pnl_total,
// the raw pre-exclusion row.amount) and what General Overheads is
// actually carrying (total, the post-exclusion row.active_amount -
// unchanged, this is the figure every existing consumer of this
// function already relies on). Additive only - no existing field
// changed or removed.
export function build_general_overhead_category_totals(
  ...
  for (const row of rows ?? []) {
    ...
    totals.set(category.key, {
      ...
      pnl_total: to_number(existing?.pnl_total) + to_number(row.amount),
    });
  }
