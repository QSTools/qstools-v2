...
function get_allocation_input_amount(row, overhead_state) {
  ...
  // Use the row's net/active amount (post asset-finance-interest exclusion)
  // as the default allocatable amount, not the raw gross P&L amount - a
  // flagged row's real remaining General Overheads amount is active_amount,
  // and that is what should flow into per-row displays and pool splits
  // unless the user has manually overridden it.
  return to_number(row.active_amount ?? row.amount);
}
...
export function build_grouped_rows(
  ...
  const source_rows = (overhead_rows ?? []).map((row) => {
    ...
    return {
      ...
      // Net/active amount, not raw P&L amount - a row flagged as containing
      // asset finance interest has already had the Assets module's matched
      // finance-interest portion subtracted in active_amount. The category
      // breakdown and every downstream total in this module must be built
      // from that net figure, or the category headline (built here) will
      // show the gross P&L amount while the module's own top-line total
      // (built in generalOverheadCalculations.js) shows the net amount -
      // two different numbers for the same underlying figure.
      source_amount: to_number(row.active_amount ?? row.amount),
