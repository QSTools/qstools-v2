...
export function buildEmployeeOverheadCard({
  ...
  const linked_active_profiles = Array.isArray(employee_overhead_profiles)
  ...
  const active_profile =
    linked_active_profiles.find((profile) => profile.staff_id === selected_staff_id) ||
    null;

  const { calculated_profiles, total_employee_overheads_annual } =
    calculateEmployeeOverheadModule(linked_active_profiles);

  const selected_staff_total_annual = active_profile
    ? Number(active_profile.employee_overheads_total_annual) || 0
    : 0;
