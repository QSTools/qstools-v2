...
function buildDirectCostAccountLines(lines = [], categories = []) {
  const category_name_by_id = new Map(
    (Array.isArray(categories) ? categories : []).map((category) => [
      category.category_id,
      category.category_name || category.label || category.category_id,
    ])
  );
    ...
    .map((line, index) => {
      ...
      return {
        line_id:
          ...
          line.line_id ||
          line.id ||
          `direct-cost-account-${index}`,
        label: getLineName(line),
        amount: getLineAmount(line),
        direct_cost_category_id,
        direct_cost_category_name:
          category_name_by_id.get(direct_cost_category_id) ||
          "Review required",
      };
    })
...
export function buildRevenueCogsStatus(calculations = {}) {
  ...
  if (total_revenue === 0) {
    warnings.push({
      ...
      message: "No revenue data is flowing from P&L revenue classification.",
    });
  }
...
export function buildRevenueCogsCard(calculations = {}) {
  ...
  const direct_cost_account_lines = buildDirectCostAccountLines(
    calculations.cost_of_sales_lines,
    calculations.direct_cost_categories
  );
