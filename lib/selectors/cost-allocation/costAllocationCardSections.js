...
export function build_delivery_summary_section(context = {}) {
  ...
  return {
    ...
    // S18 6.3 - mandatory "everything assigned" coverage, additive.
    ...
    full_cost_attribution_coverage_label: format_percent(
      calculated?.full_cost_attribution_coverage_percent ?? 0
    ),
    unassigned_labour_group_count_all:
      calculated?.unassigned_labour_group_count_all ?? 0,
    unassigned_asset_count_all: calculated?.unassigned_asset_count_all ?? 0,
  };
}
