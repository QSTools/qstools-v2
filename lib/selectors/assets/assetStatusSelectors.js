...
export function buildAssetStatus({
  ...
  // v6.0 P2.4 A10 (2026-09-23): own cost + assigned asset overhead pools,
  // matching the output contract's productive_asset_recovery_rate. Was own
  // cost only (finance interest), which understated the rate.
  const productive_asset_recovery_cost_annual = productive_active_assets.reduce(
    (sum, asset) =>
      sum +
  ...
  // v6.0 P2.4 A21 (2026-09-23): productive assets with no own cost and no
  // assigned pools still add hours to the recovery rate denominator.
  const productive_assets_zero_cost = productive_active_assets.filter(
    (asset) =>
      Number(asset.total_asset_cost_annual ?? 0) +
        get_allocated_asset_overhead_cost(asset) <=
      0
  ...
  if (productive_assets_zero_cost.length > 0) {
    warnings.push(
      `Productive asset(s) with no cost and no assigned overhead pools: ${productive_assets_zero_cost
        .map((asset) => asset.asset_name || "Unnamed asset")
        .join(", ")}. Their utilisation hours still lower the productive asset recovery rate. Review cost inputs or asset type.`
    );
  }
  ...
  return {
    ...
    productive_asset_recovery_cost_annual,
    productive_asset_recovery_cost_annual_label: format_currency(
      productive_asset_recovery_cost_annual
    ),
