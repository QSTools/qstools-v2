  ...
  switch (allocation_status) {
    case "ready":
      return {
        ...
        status_label: "Ready",
        reason:
          "The visible operating groups and assigned source pools are ready for downstream recovery testing.",
      };
    ...
    case "strained":
      return {
        ...
        reason:
          "The structure may be usable, but the visible operating groups or coverage are under pressure.",
      };
    ...
    case "not_supported":
      return {
        ...
        reason:
          "The visible operating groups, source-pool assignment, or structure checks are not ready.",
      };
    ...
    default:
      return {
        ...
        reason:
          "Review the operating groups, source-pool assignments, and reconciliation checks.",
      };
  }
}
...
function get_recommended_check({
  ...
  if (!structure_valid) {
    return "Complete the operating groups before relying on this allocation setup.";
  }
...
function get_main_issue({
  ...
  if (!structure_valid) {
    return {
      ...
      message:
        "The operating groups are not currently valid enough for downstream use.",
    };
  }
...
export function build_cost_allocation_status(calculated = {}) {
  ...
  return {
    ...
    staff_in_operating_groups_count: calculated?.linked_staff_count ?? 0,
    assets_in_operating_groups_count: calculated?.linked_asset_count ?? 0,
  ...
  return {
    ...
    allocation_tests: {
      ...
      asset: {
        ...
        status_label: "Pending asset assignment layer",
        message:
          "Productive asset assignment will be reconciled against the Productive Asset Pool.",
      },
      overhead: {
        ...
        status_label: "Pending overhead assignment layer",
        message:
          "Remaining overhead distribution will be reconciled against the Overhead Pool.",
      },
    },
    ...
    delivery_summary: {
      ...
      staff_not_in_operating_groups_count:
        calculated?.unlinked_staff_count ?? 0,
      assets_in_operating_groups_count: calculated?.linked_asset_count ?? 0,
      assets_not_in_operating_groups_count:
        calculated?.unlinked_asset_count ?? 0,
      operating_groups_count: calculated?.total_operational_groups ?? 0,
