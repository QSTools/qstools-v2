...
function build_allocation_tests(calculated = {}) {
  ...
  const material_share_percent = calculated?.material_share_percent ?? 0;
  ...
  return {
    ...
    material: {
      title: "Materials / products allocation test",
      selected: material_selected,
      share_percent: material_share_percent,
      recovery_cost: calculated?.material_recovery_cost ?? 0,
      status_label: !material_selected
        ? "Not selected"
        : "Material margin not yet verified",
      message: !material_selected
        ? "This recovery plan is not asking Materials / products to carry a recovery share."
        : "Materials / products recovery is selected. Material margin is not yet verified and should be checked against job or sales evidence when available.",
      material_margin_status: get_status_label(
        calculated?.material_margin_status
      ),
    },
    ...
    unassigned: {
      ...
      status_label: unassigned_selected
        ? "Needs review"
        : "Fully assigned",
      message: unassigned_selected
        ? "A recovery share is being held outside labour, productive assets, and Materials / products. This may be intentional, but it should be confirmed rather than treated as an error."
