...
function get_status_copy(
  ...
  if (setup_warnings_count > 0) {
    return {
      ...
      reason:
        "Complete the setup checklist before treating the working-unit structure as fully tested.",
    };
  }

  if (structural_warnings_count > 0) {
    return {
      headline: "Working-unit structure needs review.",
      status_label: "Structure needs review",
      reason:
        "The setup checklist is clear, but structural warnings remain in the working-unit structure.",
    };
  }
  ...
  switch (allocation_status) {
    case "ready":
      return {
        headline: "This recovery plan is supported by the working units.",
        status_label: "Supported internally",
        reason:
          "The selected recovery plan is supported by the visible working units and productive capacity.",
      };
    ...
    case "ready_with_dependency":
      return {
        ...
        reason:
          "The selected recovery plan can work, but it depends on external delivery capacity beyond the visible working units.",
      };
    ...
    case "strained":
      return {
        ...
        reason:
          "The selected recovery plan may be possible, but the visible working units are under pressure or coverage is weak.",
      };
    ...
    case "not_supported":
      return {
        ...
        reason:
          "The selected recovery plan is not currently supported by the visible working units or confirmed external dependency.",
      };
    ...
    default:
      return {
        ...
        reason:
          "The selected recovery plan needs review before the working-unit structure can be trusted.",
      };
  }
}
...
function get_recommended_check({
  ...
  if (!structure_valid) {
    return "Complete the working units before relying on this recovery setup.";
  }

  return "Review running cost, overhead burden, and minimum recoverable rate before moving to revenue.";
}
...
function build_allocation_tests(calculated = {}) {
  ...
  return {
    ...
    unassigned: {
      title: "Overhead burden recovery test",
      ...
      message: unassigned_selected
        ? "A recovery share is being held outside direct labour and productive assets. This should be carried by the working units as overhead burden."
...
function get_main_issue({
  ...
  if (setup_warnings_count > 0) {
    return {
      ...
      message:
        "Complete the setup checklist before treating this working-unit structure as fully tested.",
    };
  }

  if (structural_warnings_count > 0) {
    return {
      title: "Working-unit structure needs review",
      message:
        "The setup checklist is clear, but structural warnings remain in the working-unit structure.",
    };
  }
  ...
  if (allocation_dependency_type === "asset_structure") {
    return {
      ...
      message:
        "This recovery plan uses asset-driven recovery, but the visible working units do not currently have enough valid productive asset structure to support it.",
    };
  }
  ...
  if (allocation_dependency_type === "external_delivery") {
    return {
      ...
      message:
        "This recovery plan depends on delivery capacity outside the visible internal working units.",
    };
  }
  ...
  if (allocation_dependency_type === "internal_capacity") {
    return {
      ...
      message:
        "This recovery plan needs more internal productive capacity than the current working units show.",
    };
  }

  if (!structure_valid) {
    return {
      title: "Working-unit structure is incomplete",
      message:
        "The working units are not currently valid enough to support this recovery plan.",
    };
  }
  ...
  if (allocation_status === "not_supported") {
    return {
      ...
      message:
        ...
        "The visible working units do not currently support this recovery plan.",
    };
  }

  return {
    title: "Working-unit recovery evidence needs review",
    message:
      "Review running cost, overhead burden, minimum recoverable rate, and setup warnings before relying on this allocation profile.",
  };
}
...
export function build_cost_allocation_status(calculated = {}) {
  ...
  return {
    ...
    staff_in_working_units_count: calculated?.linked_staff_count ?? 0,
    assets_in_working_units_count: calculated?.linked_asset_count ?? 0,
...
export function build_cost_allocation_card(calculated = {}) {
  ...
  const active_operational_groups = calculated?.active_operational_groups ?? [];

  const staff_rows = (calculated?.active_staff ?? []).map((staff) => {
    const is_in_working_unit = active_operational_groups.some((group) => {
  ...
  return {
    ...
    recovery_plan: {
      ...
      productive_asset_utilisation_hours_annual:
        calculated?.productive_asset_utilisation_hours_annual ?? 0,
      group_recovery_basis_label:
        calculated?.group_recovery_basis_label ?? "Operating hours",
      group_required_recovery_rate:
        calculated?.group_required_recovery_rate ?? 0,
    },
    ...
    delivery_summary: {
      ...
      staff_in_working_units_count: calculated?.linked_staff_count ?? 0,
      staff_not_in_working_units_count: calculated?.unlinked_staff_count ?? 0,
      assets_in_working_units_count: calculated?.linked_asset_count ?? 0,
      assets_not_in_working_units_count: calculated?.unlinked_asset_count ?? 0,
      working_units_count: calculated?.total_operational_groups ?? 0,
