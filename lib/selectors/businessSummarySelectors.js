...
export function buildBusinessSummaryStatus(calculations = {}) {
  ...
  if (activity_driver_type === "hours" && activity_driver_value === 0) {
    warnings.push({
      warning_id: "no_productive_output",
      message:
        "No recovery hours are available. Required recovery per hour cannot be calculated.",
    });
  }
...
export function buildBusinessSummaryCard(calculations = {}) {
  ...
  return {
    ...
    is_product_based: calculations.is_product_based === true,
    is_labour_based: calculations.is_labour_based !== false,
    ...
    recovery_gap_label: calculations.recovery_gap_label || "Hourly gap",
    revenue_per_unit: toNumber(calculations.revenue_per_unit),
    direct_cost_per_unit: toNumber(calculations.direct_cost_per_unit),
    margin_per_unit: toNumber(calculations.margin_per_unit),
    required_units_to_break_even: toNumber(
      calculations.required_units_to_break_even
    ),
    unit_surplus_or_shortfall: toNumber(
    ...
    // New metrics: labour burden and macro operating-hour pressure
    required_labour_burden_rate: toNumber(
      calculations.required_labour_burden_rate
    ),
    macro_required_operating_hour_rate: toNumber(
      calculations.macro_required_operating_hour_rate
    ),
    net_annual_business_open_hours: toNumber(
      calculations.net_annual_business_open_hours
    ),
    net_annual_business_open_days: toNumber(
      calculations.net_annual_business_open_days
    ),
    annual_open_weeks: toNumber(calculations.annual_open_weeks),
  };
}
