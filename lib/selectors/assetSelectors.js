...
export function buildAssetStatus({
  ...
  if (asset_type === "productive") {
    if (!has_positive_number(asset_state.available_hours_per_year)) {
      warnings.push("Available hours per year are required for productive assets.");
    }

    if (!has_positive_number(asset_state.utilisation_percent)) {
      warnings.push("Utilisation percent is required for productive assets.");
    }

    if (calculations.has_zero_productive_hours_warning) {
      warnings.push(
        "Productive asset hours are zero, so true asset cost per hour is set to 0."
      );
    }
  }

  if (asset_type === "support") {
    warnings.push("Support assets are included in cost but excluded from recovery units.");
  }
  ...
  return {
    ...
    productive_hours_label: format_number(calculations.productive_asset_hours),
    cost_per_hour_label: format_currency(calculations.true_asset_cost_per_hour),
...
export function buildAssetCard({
  ...
  const asset_rows = [...saved_assets]
    ...
    .map((asset) => ({
      ...
      productive_asset_hours: format_number(asset.productive_asset_hours || 0),
      true_asset_cost_per_hour: format_currency(asset.true_asset_cost_per_hour || 0),
  ...
  return {
    ...
    summary: {
      rows: [
        {
          label: "Asset Type",
          value: asset_state.asset_type === "support" ? "Support" : "Productive",
        },
        {
        ...
        {
          label: "Available Hours per Year",
          value: format_number(asset_state.available_hours_per_year),
        },
        {
          label: "Utilisation",
          value: format_percent(asset_state.utilisation_percent),
        },
        {
          label: "Productive Asset Hours",
          value: format_number(calculations.productive_asset_hours),
        },
        {
          label: "True Asset Cost per Hour",
          value: format_currency(calculations.true_asset_cost_per_hour),
          emphasis: true,
        },
      ],
      meta: {
        asset_name: asset_state.asset_name || "Unnamed asset",
        asset_type: asset_state.asset_type === "support" ? "Support" : "Productive",
