...
export function buildAssetStatus({
  ...
  if (finance_fields_started) {
    ...
    if (!has_value(asset_state.finance_start_date)) {
      warnings.push("Finance start date is required when finance inputs are used.");
    }
  }
...
export function buildAssetCard({
  ...
  const other_active_assets = active_saved_assets.filter(
  ...
  const current_asset_interest_annual = current_asset_is_active
    ? Number(calculations.asset_interest_annual ?? calculations.interest_annual ?? 0)
    : 0;
  ...
  const saved_asset_cost_annual = other_active_assets.reduce(
    (sum, asset) => sum + Number(asset.total_asset_cost_annual ?? 0),
    0
  );

  const saved_asset_interest_annual = other_active_assets.reduce(
    (sum, asset) =>
      sum + Number(asset.asset_interest_annual ?? asset.interest_annual ?? 0),
    0
  );

  const saved_principal_annual = other_active_assets.reduce(
    (sum, asset) =>
      sum +
      Number(
        asset.asset_principal_repayment_annual ?? asset.principal_annual ?? 0
      ),
    0
  );

  const saved_finance_payment_annual = other_active_assets.reduce(
    (sum, asset) =>
      sum + Number(asset.asset_total_finance_payment_annual ?? 0),
    0
  );

  const portfolio_total_asset_cost_annual =
    saved_asset_cost_annual + current_asset_cost_annual;

  const portfolio_total_asset_interest_annual =
    saved_asset_interest_annual + current_asset_interest_annual;

  const portfolio_total_principal_annual =
    saved_principal_annual + current_principal_annual;

  const portfolio_total_finance_payment_annual =
    saved_finance_payment_annual + current_finance_payment_annual;
    ...
    summary: {
      portfolio_summary: {
        rows: [
          ...
          {
            label: "Current asset preview",
            value: current_asset_has_live_value ? "Included" : "No live cost yet",
          },
          {
            label: "Annual operating asset cost",
            value: format_currency(portfolio_total_asset_cost_annual),
            emphasis: true,
          },
          {
            label: "Annual finance interest",
            value: format_currency(portfolio_total_asset_interest_annual),
          },
          {
            label: "Annual principal repayment",
            value: format_currency(portfolio_total_principal_annual),
          },
          {
            label: "Annual finance payment",
            value: format_currency(portfolio_total_finance_payment_annual),
            emphasis: true,
          },
        ],
        note:
          "Interest is included in operating asset cost. Principal is shown for cash-flow visibility only.",
      },
