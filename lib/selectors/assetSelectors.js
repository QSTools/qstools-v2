...
function build_asset_overhead_pool_summary({
  ...
  return ASSET_POOL_DEFINITIONS.map((pool) => {
    const available_amount = Number(
      asset_overhead_pools?.[pool.key]?.amount ?? 0
    );
    ...
    const assigned_to_other_assets = active_saved_assets
      ...
      .reduce((sum, asset) => {
        ...
        return sum + Number(assignments?.[pool.key] || 0);
      }, 0);

    const assigned_to_current_asset = Number(
      current_assignments?.[pool.key] || 0
    );
    ...
    return {
      ...
      assigned_total,
      remaining_amount: available_amount - assigned_total,
      ...
      remaining_amount_label: format_currency(available_amount - assigned_total),
    };
  });
}
  ...
  return {
    ...
    summary: {
      portfolio_summary: {
        ...
        note:
          "Interest is included in operating asset cost. Principal is shown for cash-flow visibility only. Asset overhead pools are recovery modelling values from General Overheads.",
      },
      ...
      rows: [
        ...
        },
        {
          ...
          value:
            asset_state.asset_type === "productive"
              ? `${format_currency(calculations.required_asset_recovery_rate)} / hr`
              : "-",
          emphasis: true,
        },
      ],
