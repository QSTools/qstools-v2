...
export function buildAssetStatus({
  ...
  const total_asset_cost_annual = Number(
  ...
  if (!no_active_assets_ready && !has_value(asset_state.asset_name)) {
    warnings.push("Asset name is missing.");
  }

  if (!no_active_assets_ready && !has_value(asset_state.effective_from)) {
  ...
  if (active_asset_count === 0 && !no_active_assets_confirmed) {
    warnings.push("No active assets are saved yet.");
  }

  const assets_ready = no_active_assets_ready
    ? true
    : warnings.length === 0 && calculations.assets_ready;
