...
export function buildLabourStatus({
  ...
  if (reconciliation.labour_status === "amber") {
    warnings.push("Labour is within amber reconciliation range and needs explanation.");
  }
  ...
  return {
    ...
    pnl_benchmark_total_label: format_currency(reconciliation.pnl_benchmark_total),
    module_total_label: format_currency(reconciliation.module_total),
    labour_variance_amount_label: format_currency(reconciliation.labour_variance_amount),
...
export function buildLabourSummary({
  ...
  return {
    ...
    rows: [
      ...
      {
        label: "Total Labour Cost Annual",
