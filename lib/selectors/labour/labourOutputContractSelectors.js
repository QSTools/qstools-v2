...
export function buildLabourOutputContract({
  ...
  // total_productive_output is defined below, after the productive staff type rates,
  // so it uses the same productive-staff-only basis as total_productive_labour_hours (F12).
  ...
  const total_productive_output = total_productive_labour_hours;
  ...
  const total_available_hours_before_productivity = active_staff.reduce(
    (total, staff) => {
      // Productive staff only, matching total_productive_output (F12).
      const is_productive_staff =
        staff?.is_productive === true || staff?.labour_class === "productive";
      return is_productive_staff
        ? total + get_available_hours_before_productivity(staff)
        : total;
    },
    0
  );
