...
export function buildModuleReconciliationChecks({
  ...
  // Broad P&L asset/equipment benchmark.
  // Keep this for diagnostics only. Do not use it as the direct asset finance benchmark.
  const assets_benchmark_total = toNumber(
    pnl_output_contract.assets_benchmark_total,
  );

  const general_overheads_benchmark_total = toNumber(
    pnl_output_contract.general_overheads_benchmark_total,
  );

  // Correct P&L-side benchmark for direct asset finance reconciliation.
  ...
  // Correct Assets-side total for direct asset finance reconciliation.
  ...
  const normalised_reconciliation_inputs = {
    ...
    // Broad asset/equipment benchmark retained for diagnostic visibility only.
    assets_benchmark_total,

    // Direct asset finance benchmark used by Model Readiness asset finance card.
    ...
    // Broad total asset cost retained for Cost Summary / total business cost context.
    total_asset_cost_annual,

    // Direct Assets-side finance/interest total.
  ...
  return {
    ...
    // P&L benchmark component totals
    labour_benchmark_total,

    // Broad asset/equipment benchmark. Diagnostic only.
    assets_benchmark_total,

    // Correct P&L benchmark for asset finance reconciliation.
    asset_finance_benchmark_total,

    general_overheads_benchmark_total,

    // Module source component totals
    total_labour_cost_annual,

    // Broad total asset cost. Used for Cost Summary context.
    total_asset_cost_annual,

    // Correct Assets module finance/interest total.
    total_asset_interest_annual,
