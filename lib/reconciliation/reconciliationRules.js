...
function buildVarianceCheck({
  ...
  return {
    ...
    module,
    label,
    severity: has_material_variance ? "warning" : "info",
    passed: !has_material_variance,
    message: has_material_variance
      ? `${module} does not reconcile to its P&L benchmark.`
      : `${module} reconciles to its P&L benchmark.`,
    detail: has_material_variance
      ? "This is evidence of a difference, not automatically an error. See possible reasons."
      : "No material unexplained variance detected.",
    possible_reasons: has_material_variance ? possible_reasons : [],
    status: has_material_variance ? "warn" : "pass",
    is_blocking: false,
    is_warning: has_material_variance,
    source_amount,
    module_amount,
    variance_amount,
    variance_percent,
