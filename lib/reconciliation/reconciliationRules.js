...
// S22 (this session): a P&L interest line was actively reviewed and
// marked "does not contain asset finance interest" - a decision, not
// an absence of data - while Assets still shows real finance interest
// cost. Distinct from the timing-lag case: nobody being wrong yet is
// different from someone having already looked and answered "no".
const ASSET_FINANCE_MISCLASSIFICATION_POSSIBLE_REASONS = [
  "A P&L interest line has been reviewed and marked as not containing asset finance interest, but the Assets module shows real, confirmed finance interest cost",
  "This P&L line may be misclassified - check its Interest Classification",
  "Until corrected, this asset finance interest cost may be counted in General Overheads as well as in Assets, a genuine double-count",
];
...
function getAssetFinanceCoverageCheck({
  ...
  const remaining_uncovered_amount = Math.max(-variance_amount, 0);

  // S22 (this session): "Covered" is a true statement, but on its own
  // it reads as settled. It is not - the remaining amount is currently
  // ASSUMED to be genuine other interest, an assumption nobody has
  // confirmed yet. Naming the actual dollar figure and pointing at the
  // Assets card (which stays open/amber for exactly this reason) makes
  // that assumption visible here too, not just on the Assets page.
  const covered_message =
    remaining_uncovered_amount > ASSET_FINANCE_COVERAGE_TOLERANCE
      ? `Your Assets module's finance interest is fully covered within your P&L's total interest. The remaining ${remaining_uncovered_amount.toLocaleString(undefined, { maximumFractionDigits: 0 })} has already been carried into General Overheads as other interest cost - this is already accounted for, nothing further to do here.`
      : "Your Assets module's finance interest is fully covered within your P&L's total interest.";
...
function getAssetFinanceMisclassificationCheck({
  total_asset_interest_annual,
}) {
  return {
    id: "asset_finance_variance",
    ...
    passed: false,
    message:
      "A P&L interest line has been reviewed and marked as not containing asset finance interest, but the Assets module shows real finance interest cost that is not matched anywhere on the P&L.",
    detail:
      "This is not a timing lag - a P&L interest line has been actively reviewed, not left unreviewed. Check whether that line's Interest Classification is correct.",
    possible_reasons: ASSET_FINANCE_MISCLASSIFICATION_POSSIBLE_REASONS,
    status: "warn",
    is_blocking: false,
    is_warning: true,
    is_timing_expected: false,
    is_coverage_check: false,
    tolerance_basis: "dollar_floor",
    source_amount: 0,
    module_amount: total_asset_interest_annual,
    variance_amount: total_asset_interest_annual,
    variance_percent: 0,
  };
}
...
function getAssetFinanceVarianceCheck({
  ...
  reviewed_non_asset_finance_interest_total,
}) {
  ...
  // S22 (this session): a reviewed-and-rejected P&L interest line is a
  // human decision, not an absence of data - different from nobody
  // having reviewed it yet, which the timing-aware fallback below
  // assumes. If Assets shows real interest with nothing flagged, and
  // at least one interest line has been actively marked "does not
  // contain asset finance interest", this is not a timing lag.
  const has_reviewed_rejection =
    toNumber(reviewed_non_asset_finance_interest_total) > 0;

  if (
    has_reviewed_rejection &&
    total_asset_interest_annual > ASSET_FINANCE_COVERAGE_TOLERANCE
  ) {
    return getAssetFinanceMisclassificationCheck({
      total_asset_interest_annual,
    });
  }
...
export function buildModuleReconciliationChecks({
  ...
  const reviewed_non_asset_finance_interest_total = toNumber(
    pnl_output_contract.reviewed_non_asset_finance_interest_total,
  );
