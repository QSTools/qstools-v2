...
function should_exclude_interest_line(line) {
  return line?.interest_treatment === "asset_finance_exclude";
}
...
export function calculateProfitAndLoss(state = {}) {
  ...
  // Section totals - accounting view stays raw P&L
  ...
  const employee_overheads_benchmark_total = sum_qs_line_amounts(
  ...
  const excluded_asset_finance_interest_total = pnl_lines.reduce((sum, line) => {
    if (!should_exclude_interest_line(line)) {
      return sum;
    }
  ...
  return {
    ...
    // section totals - raw P&L
    total_trading_income,
    total_cost_of_sales,
    total_other_income,
    total_operating_expenses,

    // traditional accounting outputs - raw P&L
    gross_profit,
    net_profit,
    ...
    // QS outputs - adjusted for QS classification rules
    total_revenue,
    materials_cost,
    subcontract_cost,
