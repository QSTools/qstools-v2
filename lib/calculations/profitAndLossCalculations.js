...
// S22 (this session): distinct from "not yet reviewed". This tracks
// interest lines a human has actively reviewed and classified as NOT
// containing asset finance interest - a real decision, not an absence
// of data. reconciliationRules.js uses this to tell a genuine timing
// lag (no review yet) apart from a reviewed line that may be wrong.
function is_reviewed_non_asset_finance_interest(line) {
  return (
    normalise_interest_treatment(line?.interest_treatment) ===
...
function sum_reviewed_non_asset_finance_interest(lines = []) {
  return (lines ?? []).reduce((sum, line) => {
    if (
      !is_interest_line(line) ||
      !is_reviewed_non_asset_finance_interest(line)
    ) {
      return sum;
    }
...
export function calculateProfitAndLoss(state = {}) {
  ...
  const reviewed_non_asset_finance_interest_total =
    sum_reviewed_non_asset_finance_interest(operating_expense_lines);
  ...
  const review_required_lines = pnl_lines.filter(
    (line) => !is_empty_placeholder_line(line) && is_review_assignment_line(line),
  );

  // S22 NOTE: general_overheads_benchmark_total / assets_benchmark_total
  // are intentionally left as RAW, unadjusted P&L category sums here.
  // The asset-finance-interest exclusion/redistribution (implementing
  // brief 03 section 14.3) is calculated in
  // lib/reconciliation/reconciliationRules.js instead, NOT here -
  // because the correct redistribution amount is
  // min(excluded_asset_finance_interest_total, the Assets module's
  // real total_asset_interest_annual), and this calculation layer has
  // no visibility into the Assets module's data. Only
  // reconciliationRules.js receives both pnl_output_contract and
  // asset_output_contract simultaneously, so only it can compute the
  // correct capped redistribution. An earlier attempt redistributed
  // the FULL excluded amount here, which over-corrected General
  // Overheads whenever a flagged P&L line was larger than the Assets
  // module's real interest (a blended line with other, genuinely
  // General Overheads-owned interest mixed in) - found and corrected
  // during S22 testing.
