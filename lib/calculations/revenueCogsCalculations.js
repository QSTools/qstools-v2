...
function normalise_unit_driver_row(row = {}, index = 0) {
  return {
    ...
    estimated_units_annual: to_number(row.estimated_units_annual),
  };
}
...
function calculate_unit_driver_rows({
  ...
  const total_margin_pool = to_number(margin_pool);
  ...
  return working_rows.map((row, index) => {
    ...
    const annual_margin_pool =
      total_margin_pool * (revenue_share_percent / 100);

    const is_revenue_bucket = row.unit_type === "each";

    const derived_units_annual = is_revenue_bucket
      ? to_number(row.estimated_units_annual)
      : safe_divide(annual_revenue, row.average_sale_rate_per_unit);

    const margin_per_unit = safe_divide(
      annual_margin_pool,
    ...
    return {
      ...
      margin_per_unit,
      annual_margin_pool,
      margin_source: "p_and_l_margin_pool",
      is_revenue_bucket,
    };
  });
}
...
function build_unit_driver_warnings({
  ...
  unit_driver_rows.forEach((row) => {
    if (row.unit_type === "each") {
      if (to_number(row.estimated_units_annual) <= 0) {
        warnings.push({
          warning_id: "unit_quantity_missing",
          unit_driver_id: row.id,
          message: `${row.unit_label} needs estimated units sold per year.`,
        });
      }
    ...
    if (to_number(row.average_sale_rate_per_unit) <= 0) {
      warnings.push({
        ...
        unit_driver_id: row.id,
        message: `${row.unit_label} needs an average sale rate per measurable unit.`,
      });
    }

    if (to_number(row.derived_units_annual) <= 0) {
      warnings.push({
        warning_id: "unit_volume_not_available",
        unit_driver_id: row.id,
        message: `${row.unit_label} cannot derive annual units until the average sale rate is greater than zero.`,
      });
    }
  });
...
export function calculateRevenueCogs({
  ...
  cost_of_sales_lines: normalise_array(cost_of_sales_lines),
  direct_cost_categories: normalise_array(direct_cost_categories),
