import { calculateRateBuilderQuotePreview, get_calculator_recovery_hours } from "@/lib/calculations/rateBuilderCalculations";
...
export function calculateOperatingGroupSplit({
  ...
  const rows = operationalGroupCostRows.map((group) => {
    ...
    const recoveryDriverQuantity = get_calculator_recovery_hours( // CM-8 fix (2026-09-26): shared rule, was type === "time" (dead field)
      preview.line_totals,
