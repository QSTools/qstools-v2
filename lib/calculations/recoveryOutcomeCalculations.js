...
export function calculateRecoveryOutcome({
  ...
  const duplicate_link_warnings = asArray(
  ...
  if (share_not_balanced) {
    ...
    labour_required_for_recovery &&
    (no_productive_output || labour_recovery_without_labour)
  ) {
    outcome_status = "not_viable";
    primary_constraint_key = "no_recoverable_base";
  } else if (
    structure_required_for_recovery &&
    (!structure_valid || structure_missing)
  ) {
    outcome_status = "not_viable";
  ...
  } else if (structure_valid && structure_weak) {
    outcome_status = "marginal";
