...
function get_asset_utilisation({
  asset_type,
  ...
  if (asset_type !== "productive") {
    return {
      utilisation_hours_per_week: 0,
      utilisation_hours_annual: 0,
      annual_weeks_used: 0,
      annual_weeks_source: "not_applicable",
    };
  }

  const weekly_hours = Math.max(to_number(utilisation_hours_per_week), 0);
  const override_weeks = to_number(asset_annual_weeks_override);
  const default_weeks = to_number(business_default_annual_weeks);

  let annual_weeks_used;
  let annual_weeks_source;

  if (override_weeks > 0) {
    annual_weeks_used = override_weeks;
    annual_weeks_source = "asset_override";
  } else if (default_weeks > 0) {
    annual_weeks_used = default_weeks;
    annual_weeks_source = "opening_hours_default";
  } else {
    annual_weeks_used = 48;
    annual_weeks_source = "fallback_default";
  }

  return {
    utilisation_hours_per_week: weekly_hours,
    utilisation_hours_annual: weekly_hours * annual_weeks_used,
    annual_weeks_used,
    annual_weeks_source,
  };
}
...
function calculate_years_between_dates(start_date_value, end_date_value) {
  const start_date = parse_date(start_date_value);
  const end_date = parse_date(end_date_value);
...
function get_effective_finance_term({
  finance_start_date,
  finance_term_years,
  finance_term_extended,
  revised_finance_end_date,
  revised_term_months,
}) {
  const original_finance_end_date = calculate_finance_end_date({
    finance_start_date,
    finance_term_years,
  });

  if (!finance_term_extended) {
    return {
      original_finance_end_date,
      finance_end_date: original_finance_end_date,
      effective_finance_term_years: finance_term_years,
    };
  }

  if (revised_finance_end_date) {
    const effective_finance_term_years = calculate_years_between_dates(
      finance_start_date,
      revised_finance_end_date
    );
    ...
    return {
      original_finance_end_date,
      finance_end_date: revised_finance_end_date,
      effective_finance_term_years:
        effective_finance_term_years > 0
          ? effective_finance_term_years
          : finance_term_years,
    };
  }

  if (revised_term_months > 0) {
    const extension_years = revised_term_months / 12;
    const effective_finance_term_years = finance_term_years + extension_years;

    return {
      original_finance_end_date,
      finance_end_date: calculate_finance_end_date({
        finance_start_date,
        finance_term_years: effective_finance_term_years,
      }),
      effective_finance_term_years,
    };
  }
...
function get_finance_lifecycle({
  ...
  return {
    ...
    effective_finance_term_years,
    finance_active: true,
    finance_status: finance_term_extended ? "extended" : "active",
  };
}
...
export function calculateAssetOutputs(
  ...
  const finance_paid_off_date =
    typeof asset_state.finance_paid_off_date === "string"
      ? asset_state.finance_paid_off_date
      : "";
  const finance_term_extended = asset_state.finance_term_extended === true;
  const revised_finance_end_date =
    typeof asset_state.revised_finance_end_date === "string"
      ? asset_state.revised_finance_end_date
      : "";
  const revised_term_months = to_number(asset_state.revised_term_months);
  ...
  const utilisation = get_asset_utilisation({
    asset_type,
    utilisation_hours_per_week: asset_state.utilisation_hours_per_week,
    asset_annual_weeks_override: asset_state.asset_annual_weeks_override,
    business_default_annual_weeks,
  });
  const required_asset_recovery_rate =
    utilisation.utilisation_hours_annual > 0
      ? total_asset_cost_annual / utilisation.utilisation_hours_annual
      : 0;
  ...
  return {
    ...
    original_finance_end_date,
    revised_finance_end_date,
    effective_finance_term_years: round_percent(effective_finance_term_years),
    finance_end_date,
    finance_active,
    finance_paid_off,
    ...
    utilisation_hours_per_week: round_percent(
      utilisation.utilisation_hours_per_week
    ),
    utilisation_hours_annual: round_percent(utilisation.utilisation_hours_annual),
    asset_annual_weeks_override:
      to_number(asset_state.asset_annual_weeks_override) || null,
    annual_weeks_used: utilisation.annual_weeks_used,
    annual_weeks_source: utilisation.annual_weeks_source,
    required_asset_recovery_rate: round_currency(required_asset_recovery_rate),
    productive_asset_hours: round_percent(utilisation.utilisation_hours_annual),
    true_asset_cost_per_hour: round_currency(required_asset_recovery_rate),
