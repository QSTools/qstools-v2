...
function calculate_reconciliation_state(assets_variance_percent) {
...
export function calculateAssetOutputs(asset_state = {}, saved_assets = []) {
  const asset_type =
  ...
  const assets_benchmark_total = to_number(asset_state.assets_benchmark_total);
  ...
  // Phase 1 simplified: keep contract name, remove user-facing running cost input.
  ...
  const assets_ready =
    assets_benchmark_total > 0 && reconciliation_state === "green";
