...
function calculate_finance_breakdown_annual({
  purchase_price,
  interest_rate,
  finance_term_years,
}) {
  ...
  if (!has_finance || finance_cost_annual <= 0) {
    return {
      principal_annual: 0,
      interest_annual: 0,
    };
  ...
  const monthly_rate = interest_rate / 100 / 12;
  const monthly_repayment = finance_cost_annual / 12;

  let remaining_balance = purchase_price;
  let principal_annual = 0;
  let interest_annual = 0;

  for (let month = 0; month < 12; month += 1) {
    const interest = remaining_balance * monthly_rate;
    const principal = monthly_repayment - interest;

    interest_annual += interest;
    principal_annual += principal;
...
export function calculateAssetOutputs(
  ...
  const calculated_finance_cost_annual = calculate_finance_cost_annual({
    purchase_price,
    interest_rate,
    finance_term_years,
  });

  const calculated_finance_breakdown =
    calculate_finance_breakdown_annual({
      purchase_price,
      interest_rate,
      finance_term_years,
    });
  const principal_annual = finance_active
    ? calculated_finance_breakdown.principal_annual
    : 0;
  const interest_annual = finance_active
    ? calculated_finance_breakdown.interest_annual
    : 0;
  const finance_cost_annual = finance_active
    ? calculated_finance_cost_annual
    : 0;

  const running_cost_annual = 0;

  const total_asset_cost_annual = finance_cost_annual + running_cost_annual;
  ...
  return {
    ...
    principal_annual: round_currency(principal_annual),
    interest_annual: round_currency(interest_annual),
    finance_start_date,
    finance_end_date,
    finance_active,
    finance_paid_off,
