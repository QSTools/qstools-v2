...
// Applies MULTIPLE group rate levers at once (S26 follow-up, live combined
// headline). Same mechanics as applyGroupRateLever above, generalised from
// one target group to a map of { group_id: rate_delta_per_hour }.
export function applyGroupRateLevers({
  group_real_capacity,
  materials_naive_revenue,
  materials_true_cost,
  unassigned_total,
  ...
  const groups = (group_real_capacity || []).map((g) => {
    ...
    const revenue_delta = delta_per_hour * to_number(g.group_recovery_hours);
  ...
  const materials_final_revenue = Math.max(
    to_number(materials_naive_revenue),
    to_number(materials_true_cost)
  );
  const total_modelled_revenue = round_currency(
    groups.reduce((sum, g) => sum + g.modelled_revenue, 0) + materials_final_revenue
  );
...
export function calculateProportionalGroupTargets({
  group_real_capacity,
  materials_naive_revenue,
  materials_true_cost,
  unassigned_total,
}) {
  const groups = group_real_capacity || [];

  const current_total_revenue =
    groups.reduce((sum, g) => sum + to_number(g.modelled_revenue), 0) +
    to_number(materials_naive_revenue);
  const current_total_cost =
    groups.reduce((sum, g) => sum + to_number(g.true_cost), 0) +
    to_number(materials_true_cost);
  const current_total_net_profit = round_currency(
    current_total_revenue - current_total_cost - to_number(unassigned_total)
  );
  ...
  if (required_revenue_delta <= 0) {
    return {
      available: true,
      already_at_or_above_breakeven: true,
      required_revenue_delta,
      targets: [],
    };
  }
  ...
  const eligible = groups
    .map((g) => ({
      group_id: g.group_id,
      group_name: g.group_name,
      group_recovery_hours: to_number(g.group_recovery_hours),
      current_rate_per_hour:
        to_number(g.group_recovery_hours) > 0
          ? round_currency(to_number(g.modelled_revenue) / to_number(g.group_recovery_hours))
          : null,
      naive_net_profit: round_currency(to_number(g.modelled_revenue) - to_number(g.true_cost)),
    }))
    .filter((g) => g.naive_net_profit > 0 && g.group_recovery_hours > 0);

  const total_eligible_margin = eligible.reduce((sum, g) => sum + g.naive_net_profit, 0);

  if (total_eligible_margin <= 0) {
    return {
      available: false,
      reason: "No currently-profitable group to base a proportional fix on",
      required_revenue_delta,
      targets: [],
    };
  }

  const targets = eligible.map((g) => {
    const weight = g.naive_net_profit / total_eligible_margin;
    const suggested_revenue_delta = round_currency(required_revenue_delta * weight);
    const suggested_rate_delta_per_hour = round_currency(
      suggested_revenue_delta / g.group_recovery_hours
    );
    return {
      group_id: g.group_id,
      group_name: g.group_name,
      weight_percent: round_currency(weight * 100),
      current_rate_per_hour: g.current_rate_per_hour,
      suggested_rate_delta_per_hour,
      suggested_target_rate:
        g.current_rate_per_hour !== null
          ? round_currency(g.current_rate_per_hour + suggested_rate_delta_per_hour)
          : null,
    };
  });
  ...
  return {
    available: true,
    already_at_or_above_breakeven: false,
    required_revenue_delta,
    targets,
  };
}