...
export function buildSourceRows(per_source, overhead_categories = []) {
  ...
  // Asset-driven units (v6.0 step 5, owner 2026-09-26): the customer pays ONE
  // all-in rate. Changing labour alone only moves money between the labour and
  // machine shares, so the unit gets ONE lever (the all-in rate) and its labour
  // rows are shown but not leverable.
  const asset_driven_unit_ids = new Set(
    (per_source.asset_groups || [])
      .filter((g) => (g.assets || []).some((a) => a.is_asset_driven === true))
      .map((g) => g.group_id)
  );
  const unit_labour = {};
  ...
  (per_source.labour_groups || []).forEach((g) => {
    (g.staff || []).forEach((s) => {
      ...
      const in_unit_rate = asset_driven_unit_ids.has(g.group_id);
      const status = in_unit_rate ? "in_unit_rate" : row_status(basis, achieved_revenue, "no_covered_hours");
      if (in_unit_rate) {
        unit_labour[g.group_id] = unit_labour[g.group_id] || { achieved: 0, floor_cost: 0, names: [] };
        unit_labour[g.group_id].achieved += achieved_revenue;
        unit_labour[g.group_id].floor_cost += n(s.true_cost) + n(s.non_productive_share);
        unit_labour[g.group_id].names.push(s.staff_type_name);
      }
      rows.push({
        in_unit_rate,
  ...
  (per_source.asset_groups || []).forEach((g) => {
    ...
    if (asset_driven_unit_ids.has(g.group_id)) {
      const labour = unit_labour[g.group_id] || { achieved: 0, floor_cost: 0, names: [] };
      const unit_achieved = achieved_revenue + labour.achieved;
      const unit_floor_cost =
        assets.reduce((sum, a) => sum + n(a.true_cost) + n(a.non_productive_share), 0) + labour.floor_cost;
      const all_in_rate = assets[0].blended_rate;
      const unit_status = row_status(basis, unit_achieved, "no_covered_hours");
      rows.push({
        row_id: "unit:" + g.group_id,
        kind: "unit",
        unit_id: g.group_id,
        unit_name: g.group_name,
        source_name: [...labour.names, ...assets.map((a) => a.asset_name)].join(" + "),
        label: g.group_name + " / whole unit",
        basis,
        basis_unit: "hours",
        achieved_revenue: r2(unit_achieved),
        start_rate: basis > 0 ? unit_achieved / basis : null,
        floor_rate: basis > 0 ? unit_floor_cost / basis : null,
        group_min_rate: n(assets[0].minimum_recoverable_rate_per_hour),
        ceiling_rate: all_in_rate === null || all_in_rate === undefined ? null : n(all_in_rate),
        labour_mix_charge_rate: n(assets[0].labour_mix_charge_rate),
        status: unit_status,
        leverable: unit_status === "active" || unit_status === "carrying_loss",
      });
      return;
    }
  ...
  return {
    ...
    achieved_total: r2(rows.filter((r) => !r.in_unit_rate).reduce((sum, r) => sum + r.achieved_revenue, 0)),
  };
}