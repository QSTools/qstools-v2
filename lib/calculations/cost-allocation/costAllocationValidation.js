...
export function get_duplicate_link_warnings(active_links) {
  ...
  safe_array(active_links).forEach((link) => {
...
export function get_structure_valid({
  ...
  productive_labour_group_count = 0,
  assigned_labour_group_count = 0,
  productive_asset_count = 0,
  assigned_productive_asset_count = 0,
}) {
  if (safe_array(duplicate_link_warnings).length > 0) {
    return false;
  }
  ...
  const has_productive_labour =
    productive_labour_group_count > 0 && assigned_labour_group_count > 0;

  const has_productive_assets =
    productive_asset_count > 0 && assigned_productive_asset_count > 0;

  if (recovery_model === "labour_led" || recovery_model === "material_led") {
    return has_productive_labour || total_operational_groups > 0;
  }

  if (recovery_model === "asset_led") {
    return has_productive_assets;
  }

  if (recovery_model === "hybrid") {
    if (asset_recovery_selected) {
      return has_productive_assets && total_operational_groups > 0;
    }

    return has_productive_labour || has_productive_assets;
  }
