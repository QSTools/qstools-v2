...
export function calculateRecoverySummary(input = {}) {
  ...
  const is_product_based =
    input.is_product_based === true || business_type === "product_based";
  const is_labour_based =
    input.is_labour_based === true || business_type !== "product_based";
  ...
  const margin_per_unit_input = to_number(input.margin_per_unit);
  ...
  const unit_surplus_or_shortfall =
    units_sold_annual - required_units_to_break_even;
  const product_recovery_ready = margin_per_unit > 0 && units_sold_annual > 0;
