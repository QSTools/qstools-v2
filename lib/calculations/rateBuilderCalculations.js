...
export function calculateRateBuilderRecoveryPreview({
  ...
  const recovery_cost_per_output_unit =
    safe_output_driver_quantity > 0
      ? round_currency(safe_selected_recovery_cost / safe_output_driver_quantity)
      : 0;
  ...
  if (safe_total_charge > 0 && safe_selected_recovery_cost > 0) {
    recovery_status =
  ...
  return {
    ...
    selected_recovery_cost: safe_selected_recovery_cost,
    profit_amount,
