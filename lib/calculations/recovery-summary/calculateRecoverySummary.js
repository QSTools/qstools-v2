import {
  BALANCE_TOLERANCE,
  get_product_unit_display,
  is_numeric_input,
  normalise_recovery_model,
  round_currency,
  round_percent,
  to_number,
} from "./sharedRecoverySummaryHelpers.js";
import { build_overhead_absorption_diagnostics } from "./recoverySummaryOverheadDiagnostics.js";
import { build_suggested_starting_split } from "./recoverySummarySuggestedSplit.js";

export function calculateRecoverySummary(input = {}) {
  const business_summary_ready = input.business_summary_ready === true;
  const revenue_cogs = input.revenue_cogs ?? {};
  const asset_contract = input.assets ?? {};
  const labour_contract = input.labour ?? {};
  const general_overheads_contract = input.general_overheads ?? {};
  const business_type =
    input.business_type ||
    revenue_cogs.business_type ||
    (revenue_cogs.is_product_based === true ? "product_based" : "") ||
    "labour_based";
  const is_product_based =
    input.is_product_based === true ||
    revenue_cogs.is_product_based === true ||
    business_type === "product_based";
  const is_labour_based =
    input.is_labour_based === true ||
    revenue_cogs.is_labour_based === true ||
    business_type !== "product_based";
  const recovery_mode = is_product_based ? "product_unit" : "hours_based";
  const activity_driver_type = input.activity_driver_type || "hours";

  const activity_driver_label =
    input.activity_driver_label ||
    (activity_driver_type === "units"
      ? "Units sold"
      : "Selected recovery hours");

  const activity_driver_value = to_number(input.activity_driver_value);

  const required_recovery_per_driver = to_number(
    input.required_recovery_per_driver
  );

  const current_margin_per_driver = to_number(input.current_margin_per_driver);
  const recovery_gap_per_driver = to_number(input.recovery_gap_per_driver);

  const total_revenue = to_number(input.total_revenue);
  const total_direct_costs = to_number(input.total_direct_costs);
  const gross_margin_percent = to_number(input.gross_margin_percent);
  const margin_pool = to_number(input.margin_pool);
  const commercial_driver_mode =
    revenue_cogs.commercial_driver_mode ||
    (is_product_based ? "unit_based" : "hours_based");
  const unit_driver_rows = Array.isArray(revenue_cogs.unit_driver_rows)
    ? revenue_cogs.unit_driver_rows
    : [];
  const product_unit_display = get_product_unit_display({
    commercial_driver_mode,
    primary_unit_label: revenue_cogs.primary_unit_label,
    primary_unit_type: revenue_cogs.primary_unit_type,
    unit_driver_rows,
  });

  const total_people_cost_annual = to_number(
    input.total_people_cost_annual ?? labour_contract.total_labour_cost_annual
  );
  const total_asset_cost_annual = to_number(input.total_asset_cost_annual);
  const total_business_overheads = to_number(
    input.total_business_overheads ??
      general_overheads_contract.total_general_overheads
  );
  const total_cost_burden =
    to_number(input.total_cost_burden) ||
    total_people_cost_annual + total_asset_cost_annual + total_business_overheads;

  const has_productive_asset_recovery_base =
    input.has_productive_asset_recovery_base === true ||
    asset_contract.has_productive_asset_recovery_base === true;

  const productive_asset_count = to_number(
    input.productive_asset_count ?? asset_contract.productive_asset_count
  );
  const support_asset_count = to_number(
    input.support_asset_count ?? asset_contract.support_asset_count
  );
  const productive_asset_cost = to_number(
    input.productive_asset_cost ??
      input.productive_asset_cost_annual ??
      asset_contract.productive_asset_cost_annual ??
      asset_contract.productive_asset_cost
  );
  const support_asset_cost = to_number(input.support_asset_cost);
  // A11 (2026-09-23): productive_asset_cost is OWN cost only (Assets naming
  // trap). The full productive asset burden adds the General Overheads pools
  // assigned to productive assets. Business Summary does not publish the pool
  // figure, so it is read from the Assets contract (input.assets).
  // productive_asset_cost itself stays own-only - Business Outcome adds the
  // pools itself (businessOutcomeCalculations.js) and would double count.
  const productive_asset_assigned_overhead_cost = to_number(
    input.productive_asset_assigned_overhead_cost_annual ??
      asset_contract.productive_asset_assigned_overhead_cost_annual
  );
  const productive_asset_recovery_cost =
    productive_asset_cost + productive_asset_assigned_overhead_cost;
  const asset_utilisation_hours_annual = to_number(
    input.asset_utilisation_hours_annual ??
      input.total_productive_asset_utilisation_hours_annual ??
      asset_contract.total_productive_asset_utilisation_hours_annual
  );
  const required_asset_recovery_rate =
    asset_utilisation_hours_annual > 0
      ? round_currency(productive_asset_recovery_cost / asset_utilisation_hours_annual)
      : 0;

  const cost_burden_breakdown = input.cost_burden_breakdown ?? {
    people: {},
    assets: {},
    business_overheads: {},
  };

  const required_recovery_rate = to_number(input.required_recovery_rate);
  const total_recovery_hours = to_number(input.total_recovery_hours);
  const total_productive_output = to_number(input.total_productive_output);

  const recovery_hours_used =
    total_recovery_hours > 0 ? total_recovery_hours : total_productive_output;

  const actual_recovery_rate = to_number(input.actual_recovery_rate);

  const profit_or_deficit_per_recovery_hour = to_number(
    input.profit_or_deficit_per_recovery_hour
  );

  const units_sold_annual = to_number(
    revenue_cogs.total_derived_units_annual ?? input.units_sold_annual
  );
  const margin_per_unit_input = to_number(
    revenue_cogs.weighted_average_margin_per_unit ??
      revenue_cogs.margin_per_unit ??
      input.margin_per_unit
  );
  const margin_per_unit =
    margin_per_unit_input > 0 || is_numeric_input(input.margin_per_unit)
      ? margin_per_unit_input
      : units_sold_annual > 0
        ? margin_pool / units_sold_annual
        : 0;
  const total_units = units_sold_annual;
  const required_cost_per_unit =
    total_units > 0 ? round_currency(total_cost_burden / total_units) : 0;
  const unit_surplus_or_gap = round_currency(
    margin_per_unit - required_cost_per_unit
  );
  const total_annual_surplus_or_gap = round_currency(
    unit_surplus_or_gap * total_units
  );
  const required_units_to_break_even =
    margin_per_unit > 0 ? total_cost_burden / margin_per_unit : 0;
  const unit_surplus_or_shortfall = total_units - required_units_to_break_even;
  const required_units_if_margin_fixed = required_units_to_break_even;
  const required_margin_if_units_fixed = required_cost_per_unit;
  const product_recovery_ready = margin_per_unit > 0 && total_units > 0;
  const product_recovery_status =
    margin_per_unit <= 0
      ? "not_recoverable"
      : unit_surplus_or_gap < 0
        ? "shortfall"
        : "surplus";
  const product_unit_recovery_status = product_recovery_status;
  const net_position = to_number(input.net_position);
  const model_trust_state = input.model_trust_state || "blocked";

  const suggested_starting_split = build_suggested_starting_split({
    total_people_cost_annual,
    productive_asset_cost: productive_asset_recovery_cost,
    has_productive_asset_recovery_base,
    total_direct_costs,
    recovery_hours_used,
    total_productive_output,
  });

  const has_valid_recovery_hour_result =
    recovery_hours_used > 0 &&
    required_recovery_rate > 0 &&
    total_cost_burden > 0 &&
    is_numeric_input(input.actual_recovery_rate) &&
    is_numeric_input(input.profit_or_deficit_per_recovery_hour) &&
    is_numeric_input(input.margin_pool);
  const has_valid_product_result =
    is_product_based &&
    units_sold_annual > 0 &&
    total_cost_burden > 0 &&
    is_numeric_input(margin_per_unit) &&
    is_numeric_input(unit_surplus_or_shortfall) &&
    is_numeric_input(input.margin_pool);

  const recovery_model = normalise_recovery_model(
    input.recovery_model ?? suggested_starting_split.suggested_recovery_model
  );

  const active_recovery_model = recovery_model;

  const labour_share_percent = to_number(
    input.labour_share_percent ??
      suggested_starting_split.suggested_labour_share_percent
  );

  const asset_share_percent = to_number(
    input.asset_share_percent ??
      suggested_starting_split.suggested_asset_share_percent
  );

  const material_share_percent = to_number(
    input.material_share_percent ??
      suggested_starting_split.suggested_material_share_percent
  );

  const explained_recovery_total =
    labour_share_percent + asset_share_percent + material_share_percent;

  const explained_recovery_exceeds_100 =
    explained_recovery_total > 100 + BALANCE_TOLERANCE;

  const overhead_absorbed_percent = explained_recovery_exceeds_100
    ? 0
    : Math.max(0, 100 - explained_recovery_total);

  const share_total = explained_recovery_total + overhead_absorbed_percent;
  const share_not_balanced = explained_recovery_exceeds_100;

  let labour_recovery_cost = round_currency(
    total_cost_burden * (labour_share_percent / 100)
  );

  let asset_recovery_cost = round_currency(
    total_cost_burden * (asset_share_percent / 100)
  );

  let material_recovery_cost = round_currency(
    total_cost_burden * (material_share_percent / 100)
  );

  let overhead_absorbed_cost = round_currency(
    total_cost_burden * (overhead_absorbed_percent / 100)
  );

  const material_margin_pool = round_currency(margin_pool);
  const material_margin_percent =
    total_revenue > 0 ? round_percent((margin_pool / total_revenue) * 100) : 0;
  const labour_recovery_hours = round_currency(recovery_hours_used);
  const labour_recovery_status =
    total_people_cost_annual <= 0
      ? "missing_labour_cost"
      : recovery_hours_used <= 0
        ? "missing_recovery_hours"
        : "ready";
  const asset_recovery_status =
    productive_asset_count <= 0 && productive_asset_recovery_cost <= 0
      ? "no_productive_assets"
      : asset_utilisation_hours_annual <= 0
        ? "missing_utilisation"
        : "ready";

  if (recovery_mode === "hours_based") {
    labour_recovery_cost = round_currency(total_people_cost_annual);
    asset_recovery_cost = round_currency(productive_asset_recovery_cost);
    material_recovery_cost = 0;
    overhead_absorbed_cost = round_currency(total_business_overheads);
  }

  if (recovery_mode === "product_unit") {
    labour_recovery_cost = 0;
    asset_recovery_cost = 0;
    material_recovery_cost = 0;
    overhead_absorbed_cost = round_currency(total_cost_burden);
  }

  const no_productive_output = total_productive_output <= 0;
  const no_recovery_hours = recovery_hours_used <= 0;

  const required_labour_recovery_rate = no_recovery_hours
    ? 0
    : round_currency(labour_recovery_cost / recovery_hours_used);
  const labour_recovery_gap =
    recovery_mode === "hours_based" && no_recovery_hours
      ? 0
      : round_currency(current_margin_per_driver - required_labour_recovery_rate);

  const required_labour_recovery_rate_per_recovery_hour = no_recovery_hours
    ? 0
    : round_currency(labour_recovery_cost / recovery_hours_used);

  const required_asset_recovery_per_recovery_hour = no_recovery_hours
    ? 0
    : round_currency(asset_recovery_cost / recovery_hours_used);

  const required_material_recovery_per_recovery_hour = no_recovery_hours
    ? 0
    : round_currency(material_recovery_cost / recovery_hours_used);

  const overhead_absorbed_cost_per_recovery_hour = no_recovery_hours
    ? 0
    : round_currency(overhead_absorbed_cost / recovery_hours_used);

  const required_asset_recovery = round_currency(asset_recovery_cost);
  const required_material_recovery = round_currency(material_recovery_cost);

  const material_recovery_included =
    recovery_mode !== "hours_based" && material_share_percent > 0;
  const asset_recovery_included =
    recovery_mode === "hours_based"
      ? productive_asset_recovery_cost > 0
      : asset_share_percent > 0;

  const gross_profit_source_status = "revenue_cogs";
  const material_margin_status =
    margin_pool <= 0
      ? "not_recoverable"
      : total_direct_costs <= 0
        ? "no_direct_costs"
        : "margin_available";
  const asset_utilisation_status = asset_recovery_status;

  const asset_recovery_without_assets =
    asset_share_percent > 0 && !has_productive_asset_recovery_base;

  const labour_recovery_without_labour =
    labour_share_percent > 0 && total_productive_output <= 0;

  const overhead_absorption_review = build_overhead_absorption_diagnostics({
    overhead_absorbed_percent,
    labour_share_percent,
    asset_share_percent,
    material_share_percent,
    total_people_cost_annual,
    total_asset_cost_annual,
    total_business_overheads,
    total_direct_costs,
    recovery_hours_used,
  });

  const recovery_stream_total =
    labour_recovery_cost +
    asset_recovery_cost +
    material_recovery_cost +
    overhead_absorbed_cost;
  const output_labour_share_percent =
    recovery_stream_total > 0
      ? (labour_recovery_cost / recovery_stream_total) * 100
      : 0;
  const output_asset_share_percent =
    recovery_stream_total > 0
      ? (asset_recovery_cost / recovery_stream_total) * 100
      : 0;
  const output_material_share_percent =
    recovery_stream_total > 0
      ? (material_recovery_cost / recovery_stream_total) * 100
      : 0;
  const output_overhead_absorbed_percent =
    recovery_stream_total > 0
      ? Math.max(
          0,
          100 -
            output_labour_share_percent -
            output_asset_share_percent -
            output_material_share_percent
        )
      : 0;

  const recovery_plan_target_per_driver = round_currency(
    required_recovery_per_driver
  );

  const recovery_plan_split = {
    labour_share_percent: round_percent(output_labour_share_percent),
    asset_share_percent: round_percent(output_asset_share_percent),
    material_share_percent: round_percent(output_material_share_percent),
    overhead_absorbed_percent: round_percent(output_overhead_absorbed_percent),

    // Backwards-compatible output for older consumers.
    overhead_share_percent: round_percent(output_overhead_absorbed_percent),
  };

  const component_required_recovery = {
    labour: {
      share_percent: round_percent(output_labour_share_percent),
      recovery_cost: labour_recovery_cost,
      required_recovery_rate: required_labour_recovery_rate,
    },
    asset: {
      share_percent: round_percent(output_asset_share_percent),
      recovery_cost: asset_recovery_cost,
      required_recovery: required_asset_recovery,
    },
    material: {
      share_percent: round_percent(output_material_share_percent),
      recovery_cost: material_recovery_cost,
      required_recovery: required_material_recovery,
    },
    overhead: {
      share_percent: round_percent(output_overhead_absorbed_percent),
      recovery_cost: overhead_absorbed_cost,
    },
  };

  const warnings = [];

  if (
    !business_summary_ready &&
    !has_valid_recovery_hour_result &&
    !has_valid_product_result
  ) {
    warnings.push("business_summary_not_ready");
  }

  if (model_trust_state !== "ready" && model_trust_state !== "warning") {
    warnings.push("upstream_model_not_ready");
  }

  if (!business_type) {
    warnings.push("missing_business_type");
  }

  if (total_revenue <= 0 && margin_pool <= 0) {
    warnings.push("missing_revenue_cogs_data");
  }

  if (share_not_balanced) {
    warnings.push("share_not_balanced");
  }

  if (overhead_absorption_review.overhead_absorption_level === "medium") {
    warnings.push("overhead_absorption_review");
  }

  if (overhead_absorption_review.overhead_absorption_level === "high") {
    warnings.push("overhead_absorption_high");
  }

  if (activity_driver_value <= 0) {
    warnings.push("no_activity_driver");
  }

  if (business_type === "labour_based" && recovery_hours_used <= 0) {
    warnings.push("no_productive_output");
  }

  if (recovery_mode === "hours_based" && total_people_cost_annual <= 0) {
    warnings.push("missing_labour_cost");
  }

  if (
    recovery_mode === "hours_based" &&
    productive_asset_count > 0 &&
    asset_utilisation_hours_annual <= 0
  ) {
    warnings.push("missing_productive_asset_utilisation");
  }

  if (business_type === "product_based" && total_units <= 0) {
    warnings.push("no_units_sold");
  }

  if (is_product_based && margin_per_unit <= 0) {
    warnings.push("no_positive_margin_per_unit");
  }

  if (is_product_based && unit_surplus_or_gap < 0) {
    warnings.push("product_unit_shortfall");
  }

  if (required_recovery_per_driver <= 0) {
    warnings.push("no_required_recovery_per_driver");
  }

  if (margin_pool < 0) {
    warnings.push("negative_margin_pool");
  }

  if (net_position < 0) {
    warnings.push("negative_net_position");
  }

  if (recovery_gap_per_driver < 0) {
    warnings.push("recovery_gap_negative");
  }

  if (material_recovery_included && recovery_mode !== "hours_based") {
    warnings.push("material_margin_unverified");
  }

  if (asset_recovery_included && recovery_mode !== "hours_based") {
    warnings.push("asset_utilisation_unverified");
  }

  if (asset_recovery_without_assets) {
    warnings.push("asset_share_without_asset_recovery_base");
  }

  if (labour_recovery_without_labour) {
    warnings.push("labour_share_without_productive_output");
  }

  return {
    business_summary_ready,
    business_type,
    recovery_mode,
    is_product_based,
    is_labour_based,
    activity_driver_type,
    activity_driver_label,
    activity_driver_value: round_currency(activity_driver_value),

    required_recovery_per_driver: round_currency(required_recovery_per_driver),
    current_margin_per_driver: round_currency(current_margin_per_driver),
    recovery_gap_per_driver: round_currency(recovery_gap_per_driver),

    recovery_plan_target_per_driver,
    recovery_plan_split,
    component_required_recovery,

    recovery_model,
    active_recovery_model,

    suggested_recovery_model:
      suggested_starting_split.suggested_recovery_model,
    suggested_labour_share_percent:
      suggested_starting_split.suggested_labour_share_percent,
    suggested_asset_share_percent:
      suggested_starting_split.suggested_asset_share_percent,
    suggested_material_share_percent:
      suggested_starting_split.suggested_material_share_percent,
    suggested_overhead_absorbed_percent:
      suggested_starting_split.suggested_overhead_absorbed_percent,
    split_source: suggested_starting_split.split_source,

    total_revenue: round_currency(total_revenue),
    total_direct_costs: round_currency(total_direct_costs),
    gross_margin_percent: round_percent(gross_margin_percent),
    margin_pool: round_currency(margin_pool),
    gross_profit: round_currency(margin_pool),

    total_people_cost_annual: round_currency(total_people_cost_annual),
    total_asset_cost_annual: round_currency(total_asset_cost_annual),
    total_business_overheads: round_currency(total_business_overheads),
    total_cost_burden: round_currency(total_cost_burden),
    cost_burden_breakdown,

    has_productive_asset_recovery_base,
    productive_asset_count,
    support_asset_count,
    productive_asset_cost: round_currency(productive_asset_cost),
    productive_asset_cost_annual: round_currency(productive_asset_cost),
    productive_asset_assigned_overhead_cost: round_currency(
      productive_asset_assigned_overhead_cost
    ),
    productive_asset_recovery_cost_annual: round_currency(
      productive_asset_recovery_cost
    ),
    support_asset_cost: round_currency(support_asset_cost),
    asset_utilisation_hours_annual: round_currency(
      asset_utilisation_hours_annual
    ),
    required_asset_recovery_rate,

    required_revenue: round_currency(total_cost_burden),
    required_recovery_rate: round_currency(required_recovery_rate),

    total_recovery_hours: round_currency(total_recovery_hours),
    recovery_hours_used: round_currency(recovery_hours_used),
    total_productive_output: round_currency(total_productive_output),

    actual_recovery_rate: round_currency(actual_recovery_rate),
    profit_or_deficit_per_recovery_hour: round_currency(
      profit_or_deficit_per_recovery_hour
    ),

    units_sold_annual: round_currency(total_units),
    total_units: round_currency(total_units),
    margin_per_unit: round_currency(margin_per_unit),
    required_cost_per_unit,
    unit_surplus_or_gap,
    total_annual_surplus_or_gap,
    required_units_to_break_even: round_currency(
      required_units_to_break_even
    ),
    required_units_if_margin_fixed: round_currency(
      required_units_if_margin_fixed
    ),
    required_margin_if_units_fixed: round_currency(
      required_margin_if_units_fixed
    ),
    unit_surplus_or_shortfall: round_currency(unit_surplus_or_shortfall),
    product_recovery_ready,
    product_recovery_status,
    product_unit_recovery_status,
    commercial_driver_mode,
    unit_driver_rows,
    ...product_unit_display,
    net_position: round_currency(net_position),
    model_trust_state,

    labour_share_percent: round_percent(output_labour_share_percent),
    asset_share_percent: round_percent(output_asset_share_percent),
    material_share_percent: round_percent(output_material_share_percent),
    overhead_absorbed_percent: round_percent(output_overhead_absorbed_percent),

    // Backwards-compatible output for older components/selectors.
    overhead_share_percent: round_percent(output_overhead_absorbed_percent),

    explained_recovery_total: round_percent(
      output_labour_share_percent +
        output_asset_share_percent +
        output_material_share_percent
    ),
    share_total: round_percent(
      output_labour_share_percent +
        output_asset_share_percent +
        output_material_share_percent +
        output_overhead_absorbed_percent
    ),

    labour_recovery_cost,
    labour_recovery_hours,
    labour_recovery_status,
    labour_recovery_gap,
    asset_recovery_status,
    asset_recovery_cost,
    material_recovery_cost,
    material_margin_pool,
    material_margin_percent,
    overhead_absorbed_cost,

    required_labour_recovery_rate,
    required_labour_recovery_rate_per_recovery_hour,
    required_asset_recovery_per_recovery_hour,
    required_material_recovery_per_recovery_hour,
    overhead_absorbed_cost_per_recovery_hour,
    required_asset_recovery,
    required_material_recovery,

    gross_profit_source_status,
    material_margin_status,
    asset_utilisation_status,
    material_recovery_included,
    asset_recovery_included,

    overhead_absorption_level:
      overhead_absorption_review.overhead_absorption_level,
    overhead_absorption_title:
      overhead_absorption_review.overhead_absorption_title,
    overhead_absorption_message:
      overhead_absorption_review.overhead_absorption_message,
    overhead_absorption_diagnostics:
      overhead_absorption_review.overhead_absorption_diagnostics,

    share_not_balanced,
    no_productive_output,
    no_recovery_hours,
    asset_recovery_without_assets,
    labour_recovery_without_labour,
    labour_share_without_productive_output: labour_recovery_without_labour,
    asset_share_without_asset_recovery_base: asset_recovery_without_assets,

    warnings,
  };
}
