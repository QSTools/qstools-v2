...
export function build_cost_allocation_inputs({
  ...
  const recovery_outputs =
    recovery_summary?.outputs ??
    recovery_summary?.recovery_outputs ??
    recovery_summary?.card?.recovery_outputs ??
    recovery_summary?.card?.outputs ??
    {};

  const labour_records =
    labour?.profiles ??
    labour?.saved_profiles ??
    labour?.active_profiles ??
    labour?.output_contract?.per_staff ??
    labour?.staff_rows ??
    [];

  const asset_records =
    assets?.profiles ??
    assets?.saved_profiles ??
    assets?.active_profiles ??
    assets?.output_contract?.per_asset ??
    assets?.asset_rows ??
    [];

  const active_staff = safe_array(labour_records).filter(
    (row) => row?.is_active,
  );

  const active_assets = safe_array(asset_records).filter(
    (row) => row?.is_active,
  );
  ...
  return {
    ...
    overhead_share_percent: safe_number(recovery_outputs?.overhead_share_percent),

    active_staff: active_staff.map((row) => ({
      staff_id: row?.staff_id ?? "",
      staff_name: row?.staff_name ?? "Unnamed staff",
      labour_class: row?.labour_class ?? "",
      is_active: Boolean(row?.is_active),
    })),

    active_assets: active_assets.map((row) => ({
...
function get_orphan_warnings({
  ...
  active_assets.forEach((asset) => {
    ...
    if (!grouped_asset_ids.has(asset?.asset_id)) {
      orphan_warnings.push(
        make_warning(`${asset?.asset_name ?? "Asset"} is not included in any active group.`),
      );
    }
  });
...
function validate_groups({
  ...
  active_groups.forEach((group) => {
    ...
    const required_staff_count = safe_number(group?.required_staff_count);
    const available_staff_count = linked_staff_ids.size || active_staff_count;
    const staff_requirement_met = available_staff_count >= required_staff_count;
    ...
    if (!staff_requirement_met) {
      group_validation_warnings.push(
        make_warning(
          `${group?.group_name ?? "Group"} is invalid: requires ${required_staff_count} staff but only ${available_staff_count} available.`,
        ),
      );
    }
...
function get_structure_valid({
  ...
  if (active_recovery_model === "hybrid") {
    return linked_staff_count > 0 || linked_asset_count > 0 || total_operational_groups > 0;
  }
