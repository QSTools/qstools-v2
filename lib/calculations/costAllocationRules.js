...
export function build_cost_allocation_inputs({
  ...
  return {
    ...
    productive_asset_cost: safe_number(
      recovery_outputs?.productive_asset_cost
    ),
...
export function calculate_cost_allocation(inputs = {}) {
  ...
  if (material_recovery_selected) {
    allocation_warnings.push(
      make_allocation_warning(
        "material_margin_not_verified",
        "Materials / products recovery is selected, but material margin is not yet verified."
      )
    );
  }
  ...
  if (overhead_absorbed_percent > 0) {
    allocation_warnings.push(
      make_allocation_warning(
        "unassigned_recovery_share",
        "The selected recovery plan includes an unassigned recovery share that still needs a clear support path."
      )
    );
  }
