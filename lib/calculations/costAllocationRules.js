...
function build_overhead_pool({
  ...
  const raw_assignments = safe_array(overhead_group_assignments).filter(
    (assignment) => assignment?.is_active !== false
  );
  ...
  const total_labour_hours = active_group_rows.reduce(
  ...
  const enriched_assignments = raw_assignments.map((assignment) => {
    const allocation_method = assignment.allocation_method || "manual_amount";
    const group_id = assignment.group_id || "";

    const group_row = active_group_rows.find((group) => {
      return group.group_id === group_id;
    });

    let assigned_overhead_amount = 0;
    let assignment_percent = safe_number(assignment.assignment_percent);

    if (allocation_method === "manual_amount") {
      assigned_overhead_amount = safe_number(assignment.assigned_amount);
    }

    if (allocation_method === "manual_percent") {
      assigned_overhead_amount =
        available_overhead_cost * (assignment_percent / 100);
    }

    if (allocation_method === "labour_cost_weighted") {
      const weight =
        total_labour_cost > 0
          ? safe_number(group_row?.assigned_labour_cost) / total_labour_cost
          : 0;
    ...
    if (allocation_method === "labour_hours_weighted") {
      const weight =
        total_labour_hours > 0
          ? safe_number(group_row?.assigned_labour_hours) / total_labour_hours
          : 0;
    ...
    if (allocation_method === "asset_burden_weighted") {
      const weight =
        total_asset_burden > 0
          ? safe_number(group_row?.assigned_asset_burden) / total_asset_burden
          : 0;
  ...
  if (over_allocated_overhead_cost > 0) {
    allocation_status = "over_allocated";
