...
function get_staff_paid_hours(staff = {}) {
  return to_number(
    staff?.paid_hours_per_year ??
      staff?.total_hours_annual ??
      staff?.annual_paid_hours ??
      staff?.paid_hours_annual
  );
}
...
export function calculateProductiveStaffTypeRates(active_staff = []) {
  ...
  const total_productive_paid_hours = productive_staff.reduce(
    (total, staff) => total + get_staff_paid_hours(staff),
    0
  );
  ...
  productive_staff.forEach((staff) => {
    ...
    const paid_hours = get_staff_paid_hours(staff);
    ...
    group.total_paid_hours += paid_hours;
  ...
  const productive_staff_type_rates = Array.from(groups.values())
    .map((group) => {
      const weighted_productivity_percent =
        group.total_paid_hours > 0
          ? (group.total_productive_hours / group.total_paid_hours) * 100
          : 0;
  ...
  return {
    ...
    weighted_all_productive_productivity_percent:
      total_productive_paid_hours > 0
        ? (total_productive_labour_hours / total_productive_paid_hours) * 100
        : 0,
...
export function calculatePnlDerivedLabourRecovery({
  revenue = 0,
  cog = 0,
  net_profit = 0,
  non_labour_overheads = 0,
  productive_staff_type_rates = [],
  total_productive_labour_hours = 0,
  total_productive_labour_cost = 0,
} = {}) {
  const safe_revenue = to_number(revenue);
  const safe_cog = to_number(cog);
  const safe_net_profit = to_number(net_profit);
  const safe_non_labour_overheads = to_number(non_labour_overheads);
  const safe_total_productive_labour_hours = to_number(
    total_productive_labour_hours
  );
  const safe_total_productive_labour_cost = to_number(
    total_productive_labour_cost
  );

  const gross_profit = safe_revenue - safe_cog;

  const labour_recovery_pool =
    gross_profit - safe_net_profit - safe_non_labour_overheads;

  const pnl_derived_recovered_labour_rate =
    safe_total_productive_labour_hours > 0
      ? labour_recovery_pool / safe_total_productive_labour_hours
      : 0;

  const pnl_labour_recovery_multiplier =
    safe_total_productive_labour_cost > 0
      ? labour_recovery_pool / safe_total_productive_labour_cost
      : 0;

  const productive_staff_type_rates_with_pnl_recovery = (
    productive_staff_type_rates ?? []
  ).map((rate) => {
    const weighted_productive_hourly_rate = to_number(
      rate?.weighted_productive_hourly_rate
    );

    const pnl_implied_recovered_rate =
      weighted_productive_hourly_rate * pnl_labour_recovery_multiplier;

    return {
      ...rate,
      pnl_implied_recovered_rate,
      pnl_recovery_gap_to_cost_rate:
        pnl_implied_recovered_rate - weighted_productive_hourly_rate,
    };
  });
...
export function calculateLabourOutputs(input = {}) {
  ...
  const leave_non_productive_hours =
    annual_leave_hours +
  ...
  // Statutory rest/meal breaks (F19). Worked days must be derived from
  // paid hours less LEAVE only, before breaks can be counted, since break
  // entitlement only accrues on days actually worked. Rest breaks are
  // paid but non-productive; meal breaks are unpaid and already excluded
  // from hours_per_week (confirmed with the business owner 2026-09-22),
  // so they are not subtracted again here.
  const worked_days_per_year =
    daily_hours > 0
      ? (paid_hours_per_year - leave_non_productive_hours) / daily_hours
      : 0;

  const jurisdiction = input.jurisdiction || "NZ";
  const break_entitlement = get_break_entitlement(daily_hours, jurisdiction);
  const paid_rest_minutes_per_day = break_entitlement.paid_rest_minutes;
  const break_hours_annual =
    worked_days_per_year * (paid_rest_minutes_per_day / 60);

  const non_productive_paid_hours =
    leave_non_productive_hours + break_hours_annual;
  ...
  const productive_labour_cost_rate =
    productive_hours > 0 ? total_labour_cost_annual / productive_hours : 0;
