...
function find_current_asset_cash_total(accounts) {
  // Cash Ratio detection (added 2026-09-12) - looks for account names
  // containing "cash" or "bank" UNDER Current Assets specifically.
  // Deliberately does NOT look under Current Liabilities, even though
  // this business's real test data has bank accounts there (likely
  // overdraft-style accounts) - a cash ratio is defined against cash
  // ASSETS, and an overdraft is a liability by nature, not cash on hand,
  // regardless of what the account name contains. If no matching
  // account is found under Current Assets, returns null so the caller
  // can report "not identifiable" rather than silently showing $0 or
  // grabbing the wrong account.
  const matches = (accounts || []).filter((a) => {
    if (a.section !== "assets") return false;
    if (String(a.subsection || "").toLowerCase() !== "current assets") return false;
  ...
  return matches.reduce((sum, a) => sum + (to_number(a.amount) ?? 0), 0);
}
...
export function calculateWorkingCapitalRatios({ totals = {}, accounts = [] } = {}) {
  ...
  const cash_total = find_current_asset_cash_total(accounts);
  const cash_ratio = cash_total !== null ? safe_divide(cash_total, current_liabilities) : null;
  ...
  return {
    ...
    cash_ratio,
    cash_ratio_available: cash_ratio !== null,
    cash_total,
    debt_to_equity,
