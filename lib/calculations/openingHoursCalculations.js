...
function calculate_daily_open_hours(day) {
  ...
  if (close_minutes === open_minutes) {
    // Same open/close time represents a full continuous 24-hour day
    // (e.g. 24hr operation, 3-shift factory) - not a zero-length day.
    gross_minutes = MINUTES_PER_DAY;
  } else if (close_minutes < open_minutes) {
    // Closing time is after midnight (e.g. 10:00 PM to 6:00 AM) -
    // wrap around to the next day instead of treating it as invalid.
    gross_minutes = close_minutes + MINUTES_PER_DAY - open_minutes;
  } else {
    gross_minutes = close_minutes - open_minutes;
  }
...
export function calculate_opening_hours(input_state = {}) {
  ...
  // Open days per year = net open hours / normal hours per open day. Net open
  // hours already allow for shutdown weeks, public holidays and part-closed days,
  // so per-day figures stay consistent with per-hour figures.
  const net_annual_business_open_days =
    standard_daily_open_hours > 0
      ? round_number(net_annual_business_open_hours / standard_daily_open_hours)
      : 0;
