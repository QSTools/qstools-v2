...
export function calculateCostSummary({
  ...
  // Opening calendar denominators for per-day and per-week display.
  const net_annual_business_open_days = to_number(
    opening_hours_data?.net_annual_business_open_days ?? 0
  );
  const annual_open_weeks = to_number(opening_hours_data?.annual_open_weeks ?? 0);
