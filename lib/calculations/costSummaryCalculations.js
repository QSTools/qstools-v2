...
export function calculateCostSummary({
  ...
  const total_productive_output = people_rows
    .filter((row) => row.labour_class === "direct_production")
    .reduce((sum, row) => sum + row.productive_hours, 0);

  const total_asset_cost_annual = toNumber(asset_data?.total_asset_cost_annual);
